...
capacity_mix = pd.read_csv(datapath+'capacity_mix.csv')

# # prep for ternary
# re_new = new_cap[['scs','load_zone','Wind_new','Solar_new','Hydro_new','emis_policy',
#                   'exist_trd','op_char','fixed_pv','Fossil_new','pol','group']].copy(deep=True)
# re_new['WSH_new'] = re_new['Wind_new'] +  re_new['Solar_new'] +  re_new['Hydro_new']
# re_new['Solar_WSH'] = re_new['Solar_new'] / re_new['WSH_new']
# re_new['Wind_WSH'] = re_new['Wind_new'] / re_new['WSH_new']
# re_new['Hydro_WSH'] = re_new['Hydro_new'] / re_new['WSH_new']

# fig = plt.figure(figsize=(16.2, 9.6))
# fig.subplots_adjust(left=0.075, right=0.85, wspace=0.6)
# reg_list = ['Region-wide','Argentina','Brazil','Chile','Paraguay','Uruguay']

# for i in range(len(reg_list)):
#     ax = fig.add_subplot(2, 3, i+1, projection="ternary", ternary_sum=100.0)
    
#     ax.grid(axis='t', linestyle=':', zorder=0)
#     ax.grid(axis='l', which='both', linestyle=':', zorder=0)
#     ax.grid(axis='r', which='both', linestyle=':', zorder=0)
    
#     pc = ax.scatter(re_new[(re_new.load_zone==reg_list[i])&(re_new.emis_policy==0)]['Solar_WSH'],
#                     re_new[(re_new.load_zone==reg_list[i])&(re_new.emis_policy==0)]['Wind_WSH'],
#                     re_new[(re_new.load_zone==reg_list[i])&(re_new.emis_policy==0)]['Hydro_WSH'],
#                     s=30,edgecolors='k', alpha=0.7,zorder=5,clip_on=False)
#     pc = ax.scatter(re_new[(re_new.load_zone==reg_list[i])&(re_new.emis_policy==1)]['Solar_WSH'],
#                     re_new[(re_new.load_zone==reg_list[i])&(re_new.emis_policy==1)]['Wind_WSH'],
#                     re_new[(re_new.load_zone==reg_list[i])&(re_new.emis_policy==1)]['Hydro_WSH'],
#                     s=30,edgecolors='k', alpha=0.7,zorder=5,clip_on=False)
    
#     ax.set_tlabel("Solar",fontsize=14,labelpad=8)
#     ax.set_llabel("Wind",fontsize=14,labelpad=8)
#     ax.set_rlabel("Hydro",fontsize=14,labelpad=8)
...
# ### GENERATION MIX CV ###
# gmix = pd.read_csv(datapath+'generation_mix.csv')
# gmix = gmix.merge(sc_df.reset_index()[['sc_name','short_name']].rename(columns={'short_name':'scs'}),on='scs')
# gmix = gmix.drop(columns='scs').rename(columns={'sc_name':'scs'})
# gmix = gmix[gmix.period==2050].reset_index().drop(columns='index')
# gmix['Battery'] *= -1
# gmix['all'] = gmix[['Battery','Biomass','Coal','Diesel','Gas','Geothermal','Hydro','Nuclear','Solar','Wind']].sum(axis=1)
# gmix['Fossil'] = gmix[['Coal','Diesel','Gas']].sum(axis=1)
# gmix['Solar_share'] = gmix['Solar'] / gmix['all']
# gmix['Wind_share'] = gmix['Wind'] / gmix['all']
# gmix['Hydro_share'] = gmix['Hydro'] / gmix['all']
# gmix['Fossil_share'] = gmix['Fossil'] / gmix['all']
# res = []
# regen = gmix.copy(deep=True)
# regen['emis_policy'] = regen.scs.str.contains('cut90p')*1
# regen['exist_trd'] = regen.scs.str.contains('t1')*1
# regen['op_char'] = regen.scs.str.split('_', expand=True)[2].str[2:].astype(int)
# regen['fixed_pv'] = (regen.op_char > 10)*1
# regen['pol'] = regen.emis_policy.astype(str) + regen.exist_trd.astype(str)
# regen['group'] = regen.emis_policy.astype(str) + regen.exist_trd.astype(str) + regen.fixed_pv.astype(str)
# regen['pol'] = regen.pol.map({'00':'NoCut,Full','01':'NoCut,Lim','10':'90%Cut,Full','11':'90%Cut,Lim'})
# reg_list = ['Region-wide','Argentina','Brazil','Chile','Paraguay','Uruguay']
# pols = regen.pol.unique().tolist()
# for i in range(len(reg_list)):
#     for j in range(len(pols)):
#         temp = regen[(regen.load_zone==reg_list[i])&(regen.pol==pols[j])].copy(deep=True)
#         try:
#             p75_1, p25_1  = np.percentile(temp['Solar_share'],75), np.percentile(temp['Solar_share'],25)
#         except:
#             p75_1,p25_1=0,0
#         try:
#             p75_2, p25_2 = np.percentile(temp['Wind_share'],75), np.percentile(temp['Wind_share'],25)
#         except:
#             p75_2,p25_2=0,0
#         try:
#             p75_3, p25_3 = np.percentile(temp['Hydro_share'],75), np.percentile(temp['Hydro_share'],25)
#         except:
#             p75_3,p25_3=0,0
#         try:
#             p75_4, p25_4 = np.percentile(temp['Fossil_share'],75), np.percentile(temp['Fossil_share'],25)
#         except:
#             p75_4,p25_4=0,0
#         res.append((reg_list[i],pols[j],p75_1,p25_1,p75_2,p25_2,p75_3,p25_3,p75_4,p25_4))
# gen_pct_df = pd.DataFrame(res,columns=['load_zone','pol','S75','S25','W75','W25','H75','H25','F75','F25'])

# ### IQR/QCD prep ###
# gpdf = gen_pct_df.copy(deep=True)
# gpdf['emis'] = gpdf.pol.map({'NoCut,Lim':'Reference','90%Cut,Lim':'Mitigation','NoCut,Full':'Reference','90%Cut,Full':'Mitigation'})
# gpdf['coord'] = gpdf.pol.map({'NoCut,Lim':'Limited','90%Cut,Lim':'Limited','NoCut,Full':'Full','90%Cut,Full':'Full'})
# gpdf['IQR_solar'] = gpdf['S75']-gpdf['S25']
# gpdf['IQR_wind'] = gpdf['W75']-gpdf['W25']
# gpdf['IQR_hydro'] = gpdf['H75']-gpdf['H25']
# gpdf['IQR_fossil'] = gpdf['F75']-gpdf['F25']
# gpdf['QCD_solar'] = (gpdf['S75']-gpdf['S25'])/(gpdf['S75']+gpdf['S25'])
# gpdf['QCD_wind'] = (gpdf['W75']-gpdf['W25'])/(gpdf['W75']+gpdf['W25'])
# gpdf['QCD_hydro'] = (gpdf['H75']-gpdf['H25'])/(gpdf['H75']+gpdf['H25'])
# gpdf['QCD_fossil'] = (gpdf['F75']-gpdf['F25'])/(gpdf['F75']+gpdf['F25'])
# gpdf=gpdf.drop(columns=['pol','S75','S25','W75','W25','H75','H25','F75','F25'])[['load_zone','emis','coord',
#                         'IQR_solar','IQR_wind','IQR_hydro','IQR_fossil','QCD_solar','QCD_wind','QCD_hydro','QCD_fossil']]
# ### IQR RATIOS for emis ###
# x3=gpdf[gpdf.emis=='Reference'].merge(gpdf[gpdf.emis=='Mitigation'],on=['load_zone','coord'],suffixes=['_ref','_miti'])
# x3['IQRR_solar'] = x3['IQR_solar_miti']/x3['IQR_solar_ref']
# x3['IQRR_wind'] = x3['IQR_wind_miti']/x3['IQR_wind_ref']
# x3['IQRR_hydro'] = x3['IQR_hydro_miti']/x3['IQR_hydro_ref']
# x3['IQRR_fossil'] = x3['IQR_fossil_miti']/x3['IQR_fossil_ref']
# x3rat = x3[['load_zone','coord','IQRR_solar','IQRR_wind','IQRR_hydro','IQRR_fossil']].copy(deep=True).fillna(0)
# x3rat['size'] = [10,10,7,7,8.2,8.2,6.2,6.2,5,5,5,5]
# ss, pp, ec = (80,350), {'Limited':'#429DED','Full':'#E4B321'}, ['#0074DA','#AD8200']*6
# labs, panlab = ['(a)','(b)','(c)','(d)'], ['Solar','Wind','Hydro','Fossil']
# yaxlab = 'Capacity Mix Uncertainty Ratio\n'+r'$IQR_{Mitigation}\ /\ IQR_{Reference}$'


# plt.rcParams.update(plt.rcParamsDefault)
# fig, ax = plt.subplots(2,2,figsize=(8.4,7))
# plt.subplots_adjust(hspace=0.28,wspace=0.25)
# sns.scatterplot(x='load_zone',y='IQRR_solar',data=x3rat,hue='coord',ax=ax[0,0],edgecolor=ec,style='coord',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_wind',data=x3rat,hue='coord',ax=ax[0,1],edgecolor=ec,style='coord',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_hydro',data=x3rat,hue='coord',ax=ax[1,0],edgecolor=ec,style='coord',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_fossil',data=x3rat,hue='coord',ax=ax[1,1],edgecolor=ec,style='coord',size='size',sizes=ss,palette=pp)
# for i in range(2):
#     for j in range(2):
#         ax[i,j].axhline(y=1,linestyle='--',color='gray',linewidth=1,zorder=0)
#         ax[i,j].set_ylim(-0.04,2.5)
...
#         else:
#             ax[i,j].set_ylabel('')
#             ax[i,j].arrow(5.18,1.05,0,.4,color='#C70000',width=0.02,head_width=0.11,head_length=.1,clip_on=False)
#             ax[i,j].arrow(5.18,0.95,0,-.4,color='#C70000',width=0.02,head_width=0.11,head_length=.1,clip_on=False)
#             ax[i,j].annotate('$Mitigation$'+'\n'+'$increases$'+'\n'+'$uncertainty$',xy=(1.02,0.445),xycoords='axes fraction',annotation_clip=False,fontsize=6,rotation=90,ha='left',va='bottom')
#             ax[i,j].annotate('$Mitigation$'+'\n'+'$decreases$'+'\n'+'$uncertainty$',xy=(1.123,0.379),xycoords='axes fraction',annotation_clip=False,fontsize=6,rotation=90,ha='right',va='top')
#         ax[i,j].tick_params(axis='x',labelsize=10)
#         ax[i,j].set_xticks(ax[i,j].get_xticks())
#         ax[i,j].set_xticklabels(['All\nRegion','ARG','BRA','CHL','PRY','URY'])
#         for lab in ax[i,j].get_xticklabels()[0:1]:
#             lab.set_fontweight('bold')
#         ax[i,j].legend([],frameon=False)
#         ax[i,j].annotate(labs[i*2 + j],xy=(0.02,0.92), xycoords='axes fraction', fontsize=12, fontweight='bold',ha='left')
#         ax[i,j].annotate(panlab[i*2 + j],xy=(0.97,0.92),xycoords='axes fraction',fontsize=12,fontweight='bold',ha='right')
# h, l = ax[0,0].get_legend_handles_labels()
# leg = ax[1,0].legend(h[1:3], [r'$Limited\ Coordination$',r'$Full\ Coordination$'],
#                title=None,loc='center',bbox_to_anchor=(1.1,-0.31),handletextpad=0.1, ncol=2, frameon=False)
# leg.legendHandles[0]._sizes = [120]
# leg.legendHandles[0].set_edgecolor('#0074DA')
# leg.legendHandles[1]._sizes = [120]
# leg.legendHandles[1].set_edgecolor('#AD8200')
# leg.texts[0].set_y(-5)
# leg.texts[1].set_y(-5)

# plt.show()
# #plt.savefig(figpath+'iqrr_emis_gen.png',dpi=400,bbox_inches='tight')




### FINAL FIGURE 3/21/25
# ### IQR RATIOS for coord ###
...
# labs, panlab = ['(a)','(b)','(c)','(d)'], ['Solar','Wind','Hydro','Fossil']
# yaxlab = 'Capacity Mix Uncertainty Ratio\n'+r'$IQR_{FullCoord}\ /\ IQR_{LimCoord}$'

# plt.rcParams.update(plt.rcParamsDefault)
# fig, ax = plt.subplots(2,2,figsize=(9,7))
# plt.subplots_adjust(hspace=0.28,wspace=0.25)
# sns.scatterplot(x='load_zone',y='IQRR_solar',data=xrat,hue='emis',ax=ax[0,0],edgecolor=ec,style='emis',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_wind',data=xrat,hue='emis',ax=ax[0,1],edgecolor=ec,style='emis',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_hydro',data=xrat,hue='emis',ax=ax[1,0],edgecolor=ec,style='emis',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_fossil',data=xrat,hue='emis',ax=ax[1,1],edgecolor=ec,style='emis',size='size',sizes=ss,palette=pp)
# for i in range(2):
#     for j in range(2):
#         ax[i,j].axhline(y=1,linestyle='--',color='gray',linewidth=1,zorder=0)
...
#             ax[i,j].annotate('$Coordination$'+'\n'+'$decreases$'+'\n'+'$uncertainty$',xy=(1.148,0.24),xycoords='axes fraction',annotation_clip=False,fontsize=6,rotation=90,ha='right',va='top')
#         ax[i,j].tick_params(axis='x',labelsize=10)
#         ax[i,j].set_xticks(ax[i,j].get_xticks())
...
#         ax[i,j].annotate(panlab[i*2 + j],xy=(0.97,0.92),xycoords='axes fraction',fontsize=12,fontweight='bold',ha='right')
# h, l = ax[0,0].get_legend_handles_labels()
# leg = ax[1,0].legend(h[1:3], [r'$Reference$',r'$Mitigation$'],
#                 title=None,loc='center',bbox_to_anchor=(1.1,-0.31),handletextpad=0.1, ncol=2, frameon=False)
# leg.legendHandles[0]._sizes = [120]
# leg.legendHandles[0].set_edgecolor('#0074DA')
# leg.legendHandles[1]._sizes = [120]
...
# plt.show()
# #plt.savefig(figpath+'iqrr_coord.png',dpi=400,bbox_inches='tight')



# ### IQR RATIOS for coord (cap additions) ###
# x1n=cv_df[cv_df.coord=='Limited'].merge(cv_df[cv_df.coord=='Full'],on=['load_zone','emis'],suffixes=['_lim','_full'])
# x1n['IQRR_solar_n'] = x1n['IQR_solar_n_full']/x1n['IQR_solar_n_lim']
# x1n['IQRR_wind_n'] = x1n['IQR_wind_n_full']/x1n['IQR_wind_n_lim']
# x1n['IQRR_hydro_n'] = x1n['IQR_hydro_n_full']/x1n['IQR_hydro_n_lim']
# x1n['IQRR_fossil_n'] = x1n['IQR_fossil_n_full']/x1n['IQR_fossil_n_lim']
# xratn = x1n[['load_zone','emis','IQRR_solar_n','IQRR_wind_n','IQRR_hydro_n','IQRR_fossil_n']].copy(deep=True).fillna(0)

# plt.rcParams.update(plt.rcParamsDefault)
# fig, ax = plt.subplots(2,2,figsize=(8,7))
# sns.scatterplot(x='load_zone',y='IQRR_solar_n',data=xratn,hue='emis',ax=ax[0,0],s=75,edgecolor=None,style='emis')
# sns.scatterplot(x='load_zone',y='IQRR_wind_n',data=xratn,hue='emis',ax=ax[0,1],s=75,edgecolor=None,style='emis')
# sns.scatterplot(x='load_zone',y='IQRR_hydro_n',data=xratn,hue='emis',ax=ax[1,0],s=75,edgecolor=None,style='emis')
# sns.scatterplot(x='load_zone',y='IQRR_fossil_n',data=xratn,hue='emis',ax=ax[1,1],s=75,edgecolor=None,style='emis')
# for i in range(2):
#     for j in range(2):
#         ax[i,j].axhline(y=1,linestyle='--',color='gray',linewidth=0.9,zorder=0)
#         ax[i,j].set_ylim(-0.02,3.3)
#         ax[i,j].set_xlabel('')
#         ax[i,j].tick_params(labelsize=6)
#         if i+j<2:
#             ax[i,j].legend([],frameon=False)
# ax[1,1].legend(title=None,bbox_to_anchor=(1.55,1.2))
# fig.suptitle('IQR ratios computed as IQR_fullcoordination / IQR_limcoordination',y=0.93)
# plt.show()
# #plt.savefig(figpath+'iqr_ratios.png',dpi=400,bbox_inches='tight')
...
# fig, ax = plt.subplots(2,2,figsize=(9,7))
# plt.subplots_adjust(hspace=0.28,wspace=0.25)
# sns.scatterplot(x='load_zone',y='IQRR_solar',data=x2rat,hue='coord',ax=ax[0,0],edgecolor=ec,style='coord',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_wind',data=x2rat,hue='coord',ax=ax[0,1],edgecolor=ec,style='coord',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_hydro',data=x2rat,hue='coord',ax=ax[1,0],edgecolor=ec,style='coord',size='size',sizes=ss,palette=pp)
# sns.scatterplot(x='load_zone',y='IQRR_fossil',data=x2rat,hue='coord',ax=ax[1,1],edgecolor=ec,style='coord',size='size',sizes=ss,palette=pp)
# for i in range(2):
#     for j in range(2):
#         ax[i,j].axhline(y=1,linestyle='--',color='gray',linewidth=1,zorder=0)
...
#             ax[i,j].annotate('$Mitigation$'+'\n'+'$decreases$'+'\n'+'$uncertainty$',xy=(1.155,0.3775),xycoords='axes fraction',annotation_clip=False,fontsize=9,rotation=90,ha='right',va='top')
#         ax[i,j].tick_params(axis='x',labelsize=10)
#         ax[i,j].set_xticks(ax[i,j].get_xticks())
#         ax[i,j].set_xticklabels(['System\nWide','ARG','BRA','CHL','PRY','URY'])
#         for lab in ax[i,j].get_xticklabels()[0:1]:
...
# plt.show()
# #plt.savefig(figpath+'iqrr_emis.png',dpi=400,bbox_inches='tight')



# ### IQR RATIOS for emis (cap additions) ###
# x2n=cv_df[cv_df.emis=='Reference'].merge(cv_df[cv_df.emis=='Mitigation'],on=['load_zone','coord'],suffixes=['_ref','_miti'])
# x2n['IQRR_solar_n'] = x2n['IQR_solar_n_miti']/x2n['IQR_solar_n_ref']
# x2n['IQRR_wind_n'] = x2n['IQR_wind_n_miti']/x2n['IQR_wind_n_ref']
# x2n['IQRR_hydro_n'] = x2n['IQR_hydro_n_miti']/x2n['IQR_hydro_n_ref']
# x2n['IQRR_fossil_n'] = x2n['IQR_fossil_n_miti']/x2n['IQR_fossil_n_ref']
# x2ratn = x2n[['load_zone','coord','IQRR_solar_n','IQRR_wind_n','IQRR_hydro_n','IQRR_fossil_n']].copy(deep=True).fillna(0)

# plt.rcParams.update(plt.rcParamsDefault)
# fig, ax = plt.subplots(2,2,figsize=(8,7))
# sns.scatterplot(x='load_zone',y='IQRR_solar_n',data=x2ratn,hue='coord',ax=ax[0,0],s=75,edgecolor=None,style='coord')
# sns.scatterplot(x='load_zone',y='IQRR_wind_n',data=x2ratn,hue='coord',ax=ax[0,1],s=75,edgecolor=None,style='coord')
# sns.scatterplot(x='load_zone',y='IQRR_hydro_n',data=x2ratn,hue='coord',ax=ax[1,0],s=75,edgecolor=None,style='coord')
# sns.scatterplot(x='load_zone',y='IQRR_fossil_n',data=x2ratn,hue='coord',ax=ax[1,1],s=75,edgecolor=None,style='coord')
# for i in range(2):
#     for j in range(2):
#         ax[i,j].axhline(y=1,linestyle='--',color='gray',linewidth=0.9,zorder=0)
#         ax[i,j].set_ylim(-0.02,3.3)
...
#         if i+j<2:
#             ax[i,j].legend([],frameon=False)
# ax[1,1].legend(title='Coordination',bbox_to_anchor=(1.47,1.24))
# fig.suptitle('IQR ratios computed as IQR_mitigation / IQR_reference',y=0.93)
# plt.show()
# #plt.savefig(figpath+'iqr_ratios_emis.png',dpi=400,bbox_inches='tight')
...
# clr_co = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# co_lb = ['Region-wide','Argentina','Brazil','Chile','Paraguay','Uruguay']
# s, m1, m2 = 40, 'x', '*'
# axs[0,0].scatter(x=cv_df[(cv_df.coord=='Limited')&(cv_df.emis=='Reference')].IQR_solar,
#                  y=cv_df[(cv_df.coord=='Full')&(cv_df.emis=='Reference')].IQR_solar,s=s,c=clr_co,marker=m1)
# axs[1,0].scatter(x=cv_df[(cv_df.coord=='Limited')&(cv_df.emis=='Reference')].IQR_wind,
#                  y=cv_df[(cv_df.coord=='Full')&(cv_df.emis=='Reference')].IQR_wind,s=s,c=clr_co,marker=m1)
# axs[0,1].scatter(x=cv_df[(cv_df.coord=='Limited')&(cv_df.emis=='Reference')].IQR_hydro,
#                  y=cv_df[(cv_df.coord=='Full')&(cv_df.emis=='Reference')].IQR_hydro,s=s,c=clr_co,marker=m1)
# axs[1,1].scatter(x=cv_df[(cv_df.coord=='Limited')&(cv_df.emis=='Reference')].IQR_fossil,
#                  y=cv_df[(cv_df.coord=='Full')&(cv_df.emis=='Reference')].IQR_fossil,s=s,c=clr_co,marker=m1)

# axs[0,0].scatter(x=cv_df[(cv_df.coord=='Limited')&(cv_df.emis=='Mitigation')].IQR_solar,
#                  y=cv_df[(cv_df.coord=='Full')&(cv_df.emis=='Mitigation')].IQR_solar,s=s,c=clr_co,marker=m2)
# axs[1,0].scatter(x=cv_df[(cv_df.coord=='Limited')&(cv_df.emis=='Mitigation')].IQR_wind,
#                  y=cv_df[(cv_df.coord=='Full')&(cv_df.emis=='Mitigation')].IQR_wind,s=s,c=clr_co,marker=m2)
# axs[0,1].scatter(x=cv_df[(cv_df.coord=='Limited')&(cv_df.emis=='Mitigation')].IQR_hydro,
#                  y=cv_df[(cv_df.coord=='Full')&(cv_df.emis=='Mitigation')].IQR_hydro,s=s,c=clr_co,marker=m2)
# axs[1,1].scatter(x=cv_df[(cv_df.coord=='Limited')&(cv_df.emis=='Mitigation')].IQR_fossil,
#                  y=cv_df[(cv_df.coord=='Full')&(cv_df.emis=='Mitigation')].IQR_fossil,s=s,c=clr_co,marker=m2)

# for i in range(2):
#     for j in range(2):
#         if j == 1:
#             axs[j,i].set_xlabel('IQR - Limited Coord')
#         if i == 0:
#             axs[j,i].set_ylabel('IQR - Full Coord')
#         # reshape
#         #lims = (np.min([axs[j,i].get_xlim()[0], axs[j,i].get_ylim()[0]]), np.max([axs[j,i].get_xlim()[1],axs[j,i].get_ylim()[1]]))
#         lims = (-0.02, .275) if not (j==0 and i==1) else (-0.01,0.1)
#         axs[j,i].set_xlim(lims)
#         axs[j,i].set_ylim(lims)
#         axs[j,i].set_aspect(1./axs[j,i].get_data_ratio())
#         axs[j,i].plot(axs[j,i].get_xlim(), axs[j,i].get_ylim(), ls="--", alpha=0.5, color='r', zorder=0)

# plt.show()
# #plt.savefig(figpath+'IQR_1to1.png',dpi=400,bbox_inches='tight')
...
# co_lb = ['Region-wide','Argentina','Brazil','Chile','Paraguay','Uruguay']
# s, m1, m2 = 40, 'x', '*'
# axs[0,0].scatter(x=qcd_df[(qcd_df.coord=='Limited')&(qcd_df.emis=='Reference')].QCD_solar,
#                   y=qcd_df[(qcd_df.coord=='Full')&(qcd_df.emis=='Reference')].QCD_solar,s=s,c=clr_co,marker=m1)
# axs[1,0].scatter(x=qcd_df[(qcd_df.coord=='Limited')&(qcd_df.emis=='Reference')].QCD_wind,
#                   y=qcd_df[(qcd_df.coord=='Full')&(qcd_df.emis=='Reference')].QCD_wind,s=s,c=clr_co,marker=m1)
# axs[0,1].scatter(x=qcd_df[(qcd_df.coord=='Limited')&(qcd_df.emis=='Reference')].QCD_hydro,
#                   y=qcd_df[(qcd_df.coord=='Full')&(qcd_df.emis=='Reference')].QCD_hydro,s=s,c=clr_co,marker=m1)
# axs[1,1].scatter(x=qcd_df[(qcd_df.coord=='Limited')&(qcd_df.emis=='Reference')].QCD_fossil,
#                   y=qcd_df[(qcd_df.coord=='Full')&(qcd_df.emis=='Reference')].QCD_fossil,s=s,c=clr_co,marker=m1)

# axs[0,0].scatter(x=qcd_df[(qcd_df.coord=='Limited')&(qcd_df.emis=='Mitigation')].QCD_solar,
#                   y=qcd_df[(qcd_df.coord=='Full')&(qcd_df.emis=='Mitigation')].QCD_solar,s=s,c=clr_co,marker=m2)
# axs[1,0].scatter(x=qcd_df[(qcd_df.coord=='Limited')&(qcd_df.emis=='Mitigation')].QCD_wind,
#                   y=qcd_df[(qcd_df.coord=='Full')&(qcd_df.emis=='Mitigation')].QCD_wind,s=s,c=clr_co,marker=m2)
# axs[0,1].scatter(x=qcd_df[(qcd_df.coord=='Limited')&(qcd_df.emis=='Mitigation')].QCD_hydro,
#                   y=qcd_df[(qcd_df.coord=='Full')&(qcd_df.emis=='Mitigation')].QCD_hydro,s=s,c=clr_co,marker=m2)
# axs[1,1].scatter(x=qcd_df[(qcd_df.coord=='Limited')&(qcd_df.emis=='Mitigation')].QCD_fossil,
#                   y=qcd_df[(qcd_df.coord=='Full')&(qcd_df.emis=='Mitigation')].QCD_fossil,s=s,c=clr_co,marker=m2)

# for i in range(2):
#     for j in range(2):
#         if j == 1:
#             axs[j,i].set_xlabel('QCD - Limited Coord')
#         if i == 0:
#             axs[j,i].set_ylabel('QCD - Full Coord')
#         # reshape
#         lims = (np.min([axs[j,i].get_xlim()[0], axs[j,i].get_ylim()[0]]), np.max([axs[j,i].get_xlim()[1],axs[j,i].get_ylim()[1]]))
#         #lims = (-0.02, .275) if not (j==0 and i==1) else (-0.01,0.1)
#         axs[j,i].set_xlim(lims)
...
# plt.show()
# #plt.savefig(figpath+'QCD_1to1.png',dpi=400,bbox_inches='tight')


# plt.rcParams.update(plt.rcParamsDefault)
# fig, ax = plt.subplots()

# plt.scatter(x=cv_df[cv_df.coord=='Limited'].CV_solar,y=cv_df[cv_df.coord=='Full'].CV_solar,s=1.5,c='r')
# plt.scatter(x=cv_df[cv_df.coord=='Limited'].CV_wind,y=cv_df[cv_df.coord=='Full'].CV_wind,s=1.5,c='cyan')
# plt.scatter(x=cv_df[cv_df.coord=='Limited'].CV_hydro,y=cv_df[cv_df.coord=='Full'].CV_hydro,s=1.5,c='b')
# plt.scatter(x=cv_df[cv_df.coord=='Limited'].CV_fossil,y=cv_df[cv_df.coord=='Full'].CV_fossil,s=1.5,c='k')
# ax.set_ylabel('CV - Full Coordination')
# ax.set_xlabel('CV - Limited Coordination')
# # reshape
# lims = (np.min([ax.get_xlim()[0], ax.get_ylim()[0]]), np.max([ax.get_xlim()[1],ax.get_ylim()[1]]))
# lims = (0, 1.4)
# ax.set_xlim(lims)
# ax.set_ylim(lims)
# ax.set_aspect(1./ax.get_data_ratio())
# ax.plot(ax.get_xlim(), ax.get_ylim(), ls="--", alpha=0.5, color='r', zorder=0)

# plt.show()
# #plt.savefig(figpath+'cv_scatter_1to1.png', bbox_inches='tight',dpi=500,facecolor="w")


# plt.rcParams.update(plt.rcParamsDefault)
# fig, ax = plt.subplots()

# plt.scatter(x=cv_df[cv_df.emis=='Reference'].CV_solar,y=cv_df[cv_df.emis=='Mitigation'].CV_solar,s=1.5,c='r')
# plt.scatter(x=cv_df[cv_df.emis=='Reference'].CV_wind,y=cv_df[cv_df.emis=='Mitigation'].CV_wind,s=1.5,c='cyan')
# plt.scatter(x=cv_df[cv_df.emis=='Reference'].CV_hydro,y=cv_df[cv_df.emis=='Mitigation'].CV_hydro,s=1.5,c='b')
# plt.scatter(x=cv_df[cv_df.emis=='Reference'].CV_fossil,y=cv_df[cv_df.emis=='Mitigation'].CV_fossil,s=1.5,c='k')
# ax.set_ylabel('CV - Mitigation')
# ax.set_xlabel('CV - Reference')
# # reshape
# lims = (np.min([ax.get_xlim()[0], ax.get_ylim()[0]]), np.max([ax.get_xlim()[1],ax.get_ylim()[1]]))
# lims = (0, 1.4)
# ax.set_xlim(lims)
...
# fig, ax = plt.subplots()
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(solpa,'load_zone',ax=ax,color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles = ['--','-','-','-','-','-']
# for line, ls in zip(ax.get_lines(), linestyles):
#     line.set_linestyle(ls)
# ax.legend(loc='center left',bbox_to_anchor=(1,0.5))
# plt.title('  CV of solar share')
# plt.ylim(-0.005,1.5)
# plt.yticks([0,.25,.5,.75,1,1.25,1.5],['0','0.25','0.5','0.75','1.0','1.25','1.5'])
# plt.show()
# #plt.savefig('../cv_solar_share.png', dpi=600, bbox_inches='tight')

# fig, ax = plt.subplots()
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(winpa,'load_zone',ax=ax,color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles = ['--','-','-','-','-','-']
# for line, ls in zip(ax.get_lines(), linestyles):
#     line.set_linestyle(ls)
# ax.legend(loc='center left',bbox_to_anchor=(1,0.5))
# plt.title('  CV of wind share')
# plt.ylim(-0.005,1.5)
# plt.yticks([0,.25,.5,.75,1,1.25,1.5],['0','0.25','0.5','0.75','1.0','1.25','1.5'])
# plt.show()
# #plt.savefig('../cv_wind_share.png', dpi=600, bbox_inches='tight')

# fig, ax = plt.subplots()
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(hydpa,'load_zone',ax=ax,color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles = ['--','-','-','-','-','-']
# for line, ls in zip(ax.get_lines(), linestyles):
#     line.set_linestyle(ls)
# ax.legend(loc='center left',bbox_to_anchor=(1,0.5))
# plt.title('  CV of hydro share')
# plt.ylim(0,0.4)
# plt.yticks([0,.1,.2,.3,.4],['0','0.1','0.2','0.3','0.4'])
# plt.show()
# #plt.savefig('../cv_hydro_share.png', dpi=600, bbox_inches='tight')

# fig, ax = plt.subplots()
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(fospa,'load_zone',ax=ax,color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles = ['--','-','-','-','-','-']
# for line, ls in zip(ax.get_lines(), linestyles):
#     line.set_linestyle(ls)
# ax.legend(loc='center left',bbox_to_anchor=(1,0.5))
# plt.title('  CV of fossil share')
# plt.ylim(-0.005,1.5)
# plt.yticks([0,.25,.5,.75,1,1.25,1.5],['0','0.25','0.5','0.75','1.0','1.25','1.5'])
# plt.show()
# #plt.savefig('../cv_fossil_share.png', dpi=600, bbox_inches='tight')
...
# # plt.subplots_adjust(wspace=0.34,hspace=0.35)
# fig, ax = plt.subplots(2,2,figsize = (10.5,8))
# plt.subplots_adjust(wspace=0.38,hspace=0.59)
# ax_list = [ax[0,0],ax[0,1],ax[1,0],ax[1,1]]
# winpa = winpa.fillna(0)
# solpa = solpa.reindex([0,1,2,3,5,4])
# winpa = winpa.reindex([0,1,2,3,5,4])
# hydpa = hydpa.reindex([0,1,2,3,5,4])
# fospa = fospa.reindex([0,1,2,3,5,4])
# panlab = ['Solar','Wind','Hydro','Fossil']

# plt.rcParams['mathtext.bf'] = 'arial:italic:bold'

# labs = ['(a)','(b)','(c)','(d)']
# titles = ['CV of '+'$\mathbf{Solar}$'+' Share','CV of '+'$\mathbf{Wind}$'+' Share',
#           'CV of '+'$\mathbf{Hydro}$'+' Share','CV of '+'$\mathbf{Fossil}$'+' Share']
# titles = ['CV of Capacity Share','CV of Capacity Share','CV of Capacity Share','CV of Capacity Share']
# # xt = [r"${No\ CO_{2}\ Target}$"+'\n'+'$\it{Limited\ Coord.}$',r"${No\ CO_{2}\ Target}$"+'\n'+'$\it{Full\ Coord.}$',
# #       r"$90\%\ CO_{2}\ Cut$"+'\n'+'$\it{Limited\ Coord.}$',r"$90\%\ CO_{2}\ Cut$"+'\n'+'$\it{Full\ Coord.}$']
# xt = [r"$\mathbfit{Limited}$"+'\n'+'$\mathbfit{Coordination}$','$\mathbfit{Full}$'+'\n'+'$\mathbfit{Coordination}$',
#       '$\mathbfit{Limited}$'+'\n'+'$\mathbfit{Coordination}$','$\mathbfit{Full}$'+'\n'+'$\mathbfit{Coordination}$']

# color6 = ['#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1','k']
# parallel_coordinates(solpa,'load_zone',ax=ax[0,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(winpa,'load_zone',ax=ax[0,1],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(hydpa,'load_zone',ax=ax[1,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(fospa,'load_zone',ax=ax[1,1],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles, linewidths = ['-','-','-','-','-','--'], [1,1,1,1,1,2.45]
# for i in range(len(ax_list)):
#     for line, ls in zip(ax_list[i].get_lines(), linestyles):
#         line.set_linestyle(ls)
#     for line, lw in zip(ax_list[i].get_lines(), linewidths):
#         line.set_linewidth(lw)
#     ax_list[i].set_ylabel(titles[i], labelpad=8, fontsize=12)
#     ax_list[i].set_xticks([0,1,2,3])
#     ax_list[i].set_xticklabels(xt, fontsize=9)
#     ax_list[i].tick_params(axis='x', which='major', pad=6)
#     if i < 2:
#         ax_list[i].set_ylim(-0.005,1.5)
#         ax_list[i].set_yticks([0,.25,.5,.75,1,1.25,1.5],['0','0.25','0.5','0.75','1.0','1.25','1.5'])
#     else:
#         ax_list[i].set_ylim(-0.0025,0.75)
#         ax_list[i].set_yticks([0,.25,.5,.75],['0','0.25','0.5','0.75'])
#     if i < 3:
#         ax_list[i].legend([], frameon=False)
#     else:
#         # ax_list[i].legend(loc='center left',bbox_to_anchor=(-1.1,-0.34), frameon=False,ncol=6)
#         ax_list[i].legend(loc='center left',bbox_to_anchor=(-1.34,-0.6), frameon=False,ncol=6, prop={'size': 11})
#     # ax_list[i].annotate(labs[i],xy=(0.91,0.9), xycoords='axes fraction', fontsize=12, fontweight='bold')
#     ax_list[i].annotate("$\it{Reference}$\n" + "$\it{(No\ CO_{2}\ Target)}$",xy=(0.17,-0.42),xycoords='axes fraction',fontsize=8.5,ha='center')
#     ax_list[i].annotate("$\it{Mitigation}$\n" + "$\it{(90\%\ CO_{2}\ Cut)}$",xy=(0.83,-0.42),xycoords='axes fraction',fontsize=8.5,ha='center')
#     ax_list[i].annotate(panlab[i],xy=(0.04,.9),xycoords='axes fraction',fontsize=14, fontweight='bold')
#     ax_list[i].annotate(labs[i],xy=(0.88,0.9), xycoords='axes fraction', fontsize=14, fontweight='bold')
# plt.show()
# #plt.savefig(figpath+'cv_4panel_.png', dpi=1200, bbox_inches='tight')
# #######################################################################################################################
...
# ### CV DIFF SCATTER PLOT ###
# solpa['soldref'] = solpa['NoCut,Full'] - solpa['NoCut,Lim']
# solpa['soldcut'] = solpa['90%Cut,Full'] - solpa['90%Cut,Lim']
# winpa['windref'] = winpa['NoCut,Full'] - winpa['NoCut,Lim']
# winpa['windcut'] = winpa['90%Cut,Full'] - winpa['90%Cut,Lim']
# hydpa['hyddref'] = hydpa['NoCut,Full'] - hydpa['NoCut,Lim']
# hydpa['hyddcut'] = hydpa['90%Cut,Full'] - hydpa['90%Cut,Lim']
# fospa['fosdref'] = fospa['NoCut,Full'] - fospa['NoCut,Lim']
# fospa['fosdcut'] = fospa['90%Cut,Full'] - fospa['90%Cut,Lim']

# ddpa = solpa[['load_zone','soldref','soldcut']].merge(winpa[['load_zone','windref','windcut']],on='load_zone')\
#                                                 .merge(hydpa[['load_zone','hyddref','hyddcut']],on='load_zone')\
#                                                 .merge(fospa[['load_zone','fosdref','fosdcut']],on='load_zone')
# ddpa1 = pd.melt(ddpa, id_vars='load_zone', value_vars=['soldref','windref','hyddref','fosdref'])
# ddpa1['m'] = ddpa1['pol'].map({'windref':1,'soldref':2,'fosdref':3,'hyddref':4})
# ddpa2 = pd.melt(ddpa, id_vars='load_zone', value_vars=['soldcut','windcut','hyddcut','fosdcut'])
# ddpa2['m'] = ddpa2['pol'].map({'windcut':1,'soldcut':2,'fosdcut':3,'hyddcut':4})

# fig, axs = plt.subplots(1,2, figsize=(6,3))
# g1 = sns.scatterplot(data=ddpa1,x='m',y='value',hue='load_zone',ax=axs[0])
# axs[0].set_xticklabels(['','Wind','Solar','Fossil','Hydro'])
# axs[0].legend(loc=(2.2,0.3), frameon=False)
# g2 = sns.scatterplot(data=ddpa2,x='m',y='value',hue='load_zone',ax=axs[1])
# axs[1].set_xticklabels(['','Wind','Solar','Fossil','Hydro'])
# axs[1].legend([], frameon=False)
# plt.show()

# fig, ax = plt.subplots(figsize=(3,3))
# g1 = sns.scatterplot(data=ddpa1[ddpa1.load_zone=='Region-wide'],x='m',y='value',color='r',ax=ax,label='Reference')
# ax.set_xticklabels(['','Wind','Solar','Fossil','Hydro'])
# g2 = sns.scatterplot(data=ddpa2[ddpa2.load_zone=='Region-wide'],x='m',y='value',color='k',ax=ax,label='Mitigation')
# ax.legend(loc=(1.2,0.3), frameon=False)
# ax.set_ylim(-.05,.05)
# plt.show()
...
#         temp = regen[(regen.load_zone==reg_list[i])&(regen.pol==pols[j])].copy(deep=True)
#         try:
#             cv1 = np.std(temp['Solar_share'])/np.mean(temp['Solar_share'])
#         except:
#             cv1=0
#         try:
#             cv2 = np.std(temp['Wind_share'])/np.mean(temp['Wind_share'])
#         except:
#             cv2=0
#         try:
#             cv3 = np.std(temp['Hydro_share'])/np.mean(temp['Hydro_share'])
#         except:
#             cv3=0
#         try:
#             cv4 = np.std(temp['Fossil_share'])/np.mean(temp['Fossil_share'])
#         except:
#             cv4=0
#         res.append((reg_list[i],pols[j],cv1,cv2,cv3,cv4))
# cvdf = pd.DataFrame(res,columns=['load_zone','pol','CV_solar','CV_wind','CV_hydro','CV_fossil'])
# solpa=cvdf.pivot(index='load_zone',columns='pol',values='CV_solar').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# winpa=cvdf.pivot(index='load_zone',columns='pol',values='CV_wind').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# hydpa=cvdf.pivot(index='load_zone',columns='pol',values='CV_hydro').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
...
# ax_list = [ax[0,0],ax[0,1],ax[1,0],ax[1,1]]
# labs = ['(a)','(b)','(c)','(d)']
# titles = ['CV of Solar Generation Share','CV of Wind Generation Share','CV of Hydro Generation Share','CV of Fossil Generation Share']
# xt = [r"${No\ CO_{2}\ Target}$"+'\n'+'$\it{Limited\ Coord.}$',r"${No\ CO_{2}\ Target}$"+'\n'+'$\it{Full\ Coord.}$',
#       r"$90\%\ CO_{2}\ Cut$"+'\n'+'$\it{Limited\ Coord.}$',r"$90\%\ CO_{2}\ Cut$"+'\n'+'$\it{Full\ Coord.}$']
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(solpa,'load_zone',ax=ax[0,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
...
# parallel_coordinates(hydpa,'load_zone',ax=ax[1,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(fospa,'load_zone',ax=ax[1,1],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles, linewidths = ['--','-','-','-','-','-'], [1.5,1,1,1,1,1]
# for i in range(len(ax_list)):
#     for line, ls in zip(ax_list[i].get_lines(), linestyles):
#         line.set_linestyle(ls)
#     for line, lw in zip(ax_list[i].get_lines(), linewidths):
...
#     ax_list[i].annotate(labs[i],xy=(0.91,0.9), xycoords='axes fraction', fontsize=12, fontweight='bold')
# plt.show()
# ### WIND AND SOLAR ALMOST IDENTICAL TO CAPACITY CV ###
# #plt.savefig('../cv_genshare_4panel.png', dpi=600, bbox_inches='tight')
...
# ax_list = [ax[0,0],ax[0,1],ax[1,0],ax[1,1]]
# titles = ['VMR of Solar Share','VMR of Wind Share','VMR of Hydro Share','VMR of Fossil Share']
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(solpa,'load_zone',ax=ax[0,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(winpa,'load_zone',ax=ax[0,1],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(hydpa,'load_zone',ax=ax[1,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
...
#     ax_list[i].set_ylim(-0.005,.13)
#     if i < 3:
#         ax_list[i].legend([], frameon=False)
#     else:
#         ax_list[i].legend(loc='center left',bbox_to_anchor=(1,0.5))
# #plt.show()
# plt.savefig('../vmr_4panel.png', dpi=600, bbox_inches='tight')



# ### TRY OTHER DISPERSION METRICS: Dispersion Coeff. (Linear Loss Function) ###
# res = []
# recap = cap_mix.copy(deep=True)
# recap['emis_policy'] = recap.scs.str.contains('cut90p')*1
# recap['exist_trd'] = recap.scs.str.contains('t1')*1
...
#         temp = recap[(recap.load_zone==reg_list[i])&(recap.pol==pols[j])].copy(deep=True)
#         try:
#             d1=(np.abs(np.median(temp['Solar_share'])-temp['Solar_share'])/np.median(temp['Solar_share'])/len(temp)).sum()
#         except:
#             d1=0
#         try:
#             d2=(np.abs(np.median(temp['Wind_share'])-temp['Wind_share'])/np.median(temp['Wind_share'])/len(temp)).sum()
#         except:
#             d2=0
#         try:
#             d3=(np.abs(np.median(temp['Hydro_share'])-temp['Hydro_share'])/np.median(temp['Hydro_share'])/len(temp)).sum()
#         except:
#             d3=0
#         try:
#             d4=(np.abs(np.median(temp['Fossil_share'])-temp['Fossil_share'])/np.median(temp['Fossil_share'])/len(temp)).sum()
#         except:
#             d4=0
#         res.append((reg_list[i],pols[j],d1,d2,d3,d4))
# cvdf = pd.DataFrame(res,columns=['load_zone','pol','Dis_solar','Dis_wind','Dis_hydro','Dis_fossil'])
# solpa=cvdf.pivot(index='load_zone',columns='pol',values='Dis_solar').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# winpa=cvdf.pivot(index='load_zone',columns='pol',values='Dis_wind').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# hydpa=cvdf.pivot(index='load_zone',columns='pol',values='Dis_hydro').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# fospa=cvdf.pivot(index='load_zone',columns='pol',values='Dis_fossil').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])

# fig, ax = plt.subplots(2,2,figsize = (12,7))
# plt.subplots_adjust(wspace=0.3,hspace=0.3)
# ax_list = [ax[0,0],ax[0,1],ax[1,0],ax[1,1]]
# titles = ['Dispersion Coeff. (Linear Loss Function): Solar Share',
#           'Dispersion Coeff. (Linear Loss Function): Wind Share',
#           'Dispersion Coeff. (Linear Loss Function): Hydro Share',
#           'Dispersion Coeff. (Linear Loss Function): Fossil Share']
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(solpa,'load_zone',ax=ax[0,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(winpa,'load_zone',ax=ax[0,1],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(hydpa,'load_zone',ax=ax[1,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
...
#     if i < 2:
#         ax_list[i].set_ylim(-0.01,3)
#         ax_list[i].set_yticks([0,.5,1,1.5,2,2.5,3],['0','0.5','1.0','1.5','2.0','2.5','3.0'])
#     else:
#         ax_list[i].set_ylim(-0.0025,0.75)
#         ax_list[i].set_yticks([0,.25,.5,.75],['0','0.25','0.5','0.75'])
#     if i < 3:
#         ax_list[i].legend([], frameon=False)
#     else:
#         ax_list[i].legend(loc='center left',bbox_to_anchor=(1,0.5), frameon=False)
# plt.show()
# #plt.savefig('../dis_4panel.png', dpi=600, bbox_inches='tight')




# ### TRY OTHER DISPERSION METRICS: Quartile Coeff. of Dispersion ###
# res = []
# recap = cap_mix.copy(deep=True)
# recap['emis_policy'] = recap.scs.str.contains('cut90p')*1
# recap['exist_trd'] = recap.scs.str.contains('t1')*1
...
#         temp = recap[(recap.load_zone==reg_list[i])&(recap.pol==pols[j])].copy(deep=True)
#         try:
#             p3,p1 = np.percentile(temp['Solar_share'],75),np.percentile(temp['Solar_share'],25)
#             q1 = ((p3-p1)/(p3+p1))
#         except:
#             q1=0
#         try:
#             p3,p1 = np.percentile(temp['Wind_share'],75),np.percentile(temp['Wind_share'],25)
#             q2 = ((p3-p1)/(p3+p1))
#         except:
#             q2=0
#         try:
#             p3,p1 = np.percentile(temp['Hydro_share'],75),np.percentile(temp['Hydro_share'],25)
#             q3 = ((p3-p1)/(p3+p1))
#         except:
#             q3=0
#         try:
#             p3,p1 = np.percentile(temp['Fossil_share'],75),np.percentile(temp['Fossil_share'],25)
#             q4 = ((p3-p1)/(p3+p1))
#         except:
#             q4=0
#         res.append((reg_list[i],pols[j],q1,q2,q3,q4))
# cvdf = pd.DataFrame(res,columns=['load_zone','pol','Qu_solar','Qu_wind','Qu_hydro','Qu_fossil'])
# solpa=cvdf.pivot(index='load_zone',columns='pol',values='Qu_solar').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# winpa=cvdf.pivot(index='load_zone',columns='pol',values='Qu_wind').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# hydpa=cvdf.pivot(index='load_zone',columns='pol',values='Qu_hydro').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# fospa=cvdf.pivot(index='load_zone',columns='pol',values='Qu_fossil').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])

# fig, ax = plt.subplots(2,2,figsize = (12,7))
# plt.subplots_adjust(wspace=0.3,hspace=0.3)
# ax_list = [ax[0,0],ax[0,1],ax[1,0],ax[1,1]]
# titles = ['Quartile Coeff. of Dispersion: Solar Share',
#           'Quartile Coeff. of Dispersion: Wind Share',
#           'Quartile Coeff. of Dispersion: Hydro Share',
#           'Quartile Coeff. of Dispersion: Fossil Share']
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(solpa,'load_zone',ax=ax[0,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(winpa,'load_zone',ax=ax[0,1],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(hydpa,'load_zone',ax=ax[1,0],color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
...
#         line.set_linewidth(lw)
#     ax_list[i].set_title(titles[i], fontsize=10)
#     ax_list[i].set_ylim(-0.01,1.02)
#     ax_list[i].set_yticks([0,.25,0.5,0.75,1],['0','0.25','0.5','0.75','1.0'])
#     if i < 3:
#         ax_list[i].legend([], frameon=False)
#     else:
#         ax_list[i].legend(loc='center left',bbox_to_anchor=(1,0.5), frameon=False)
# plt.show()
# #plt.savefig('../qu_4panel.png', dpi=600, bbox_inches='tight')
...
#         temp = recap[(recap.load_zone==reg_list[i])&(recap.pol==pols[j])].copy(deep=True)
#         try:
#             cv4 = np.std(temp['Solar_new']+temp['Solar'])/np.std(temp['WSH'])
#         except:
#             cv4=0
#         try:
#             cv5 = np.std(temp['Wind_new']+temp['Wind'])/np.std(temp['WSH'])
#         except:
#             cv5=0
#         try:
#             cv6 = np.std(temp['Hydro_new']+temp['Hydro'])/np.std(temp['WSH'])
#         except:
#             cv6=0
#         res2.append((reg_list[i],pols[j],cv4,cv5,cv6))
# cvdf = pd.DataFrame(res2,columns=['load_zone','pol','ratio_solar','ratio_wind','ratio_hydro'])
# solpa2=cvdf.pivot(index='load_zone',columns='pol',values='ratio_solar').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# winpa2=cvdf.pivot(index='load_zone',columns='pol',values='ratio_wind').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])
# hydpa2=cvdf.pivot(index='load_zone',columns='pol',values='ratio_hydro').reset_index()[['load_zone','NoCut,Lim','NoCut,Full','90%Cut,Lim','90%Cut,Full']].reindex([4,0,1,2,3,5])


# fig, ax = plt.subplots()
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(solpa2,'load_zone',ax=ax,color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles = ['--','-','-','-','-','-']
# for line, ls in zip(ax.get_lines(), linestyles):
#     line.set_linestyle(ls)
# ax.legend(loc='center left',bbox_to_anchor=(1,0.5))
# plt.title('                   std. dev. of solar capacity / std. dev. of total renewables (incl. hydro)')
# plt.ylim(-0.005,4)
# #plt.yticks([0,.25,.5,.75,1,1.25,1.5],['0','0.25','0.5','0.75','1.0','1.25','1.5'])
# plt.show()
# #plt.savefig('../ratio_solar.png', dpi=600, bbox_inches='tight')

# fig, ax = plt.subplots()
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(winpa2,'load_zone',ax=ax,color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles = ['--','-','-','-','-','-']
# for line, ls in zip(ax.get_lines(), linestyles):
#     line.set_linestyle(ls)
# ax.legend(loc='center left',bbox_to_anchor=(1,0.5))
# plt.title('                   std. dev. of wind capacity / std. dev. of total renewables (incl. hydro)')
# plt.ylim(-0.005,4)
# #plt.yticks([0,.25,.5,.75,1,1.25,1.5],['0','0.25','0.5','0.75','1.0','1.25','1.5'])
# plt.show()
# #plt.savefig('../ratio_wind.png', dpi=600, bbox_inches='tight')

# fig, ax = plt.subplots()
# color6 = ['k','#E80000','#E5A000','#00DBE1','#6E5EE7','#CF44F1']
# parallel_coordinates(hydpa2,'load_zone',ax=ax,color=color6,alpha=1,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# linestyles = ['--','-','-','-','-','-']
# for line, ls in zip(ax.get_lines(), linestyles):
#     line.set_linestyle(ls)
# ax.legend(loc='center left',bbox_to_anchor=(1,0.5))
# plt.title('                   std. dev. of hydro capacity / std. dev. of total renewables (incl. hydro)')
# plt.ylim(-0.005,0.3)
# #plt.yticks([0,.25,.5,.75,1,1.25,1.5],['0','0.25','0.5','0.75','1.0','1.25','1.5'])
# plt.show()
# #plt.savefig('../ratio_hydro.png', dpi=600, bbox_inches='tight')
...
# fig = plt.figure(figsize=(16.2, 9.6))
# fig.subplots_adjust(left=0.075, right=0.85, wspace=0.6, hspace=0.7)
# reg_list = ['Region-wide','Argentina','Brazil','Chile','Paraguay','Uruguay']
...
#     pc = ax.scatter(cap_mix[(cap_mix.load_zone==reg_list[i])&(~cap_mix.scs.str.contains('cut90'))]['Solar_share'],
#                     cap_mix[(cap_mix.load_zone==reg_list[i])&(~cap_mix.scs.str.contains('cut90'))]['Wind_share'],
#                     cap_mix[(cap_mix.load_zone==reg_list[i])&(~cap_mix.scs.str.contains('cut90'))]['Hydro_share'],
#                     s=cap_mix[(cap_mix.load_zone==reg_list[i])&(~cap_mix.scs.str.contains('cut90'))]['size'],edgecolors='k', alpha=0.7,zorder=5,clip_on=False, label='No Emissions Cut')
#     pc2 = ax.scatter(cap_mix[(cap_mix.load_zone==reg_list[i])&(cap_mix.scs.str.contains('cut90'))]['Solar_share'],
#                     cap_mix[(cap_mix.load_zone==reg_list[i])&(cap_mix.scs.str.contains('cut90'))]['Wind_share'],
#                     cap_mix[(cap_mix.load_zone==reg_list[i])&(cap_mix.scs.str.contains('cut90'))]['Hydro_share'],
#                     s=cap_mix[(cap_mix.load_zone==reg_list[i])&(cap_mix.scs.str.contains('cut90'))]['size'],edgecolors='k', alpha=0.7,zorder=5,clip_on=False, label='90% Emissions Cut')
    
#     ax.set_tlabel("Solar",fontsize=14,labelpad=8, fontweight='bold')
#     ax.set_llabel("Wind",fontsize=14,labelpad=8, fontweight='bold')
#     ax.set_rlabel("Hydro",fontsize=14,labelpad=8, fontweight='bold')
#     ax.taxis.set_major_formatter(PercentFormatter())
#     ax.laxis.set_major_formatter(PercentFormatter())
#     ax.raxis.set_major_formatter(PercentFormatter())
#     ax.set_title(reg_list[i], fontsize=22, y=1.3)

# lg = plt.legend(loc='center left', bbox_to_anchor=(1,1), frameon=False, fontsize=14)
# for handle in lg.legendHandles:
#     handle.set_sizes([200])

# plt.show()
# #plt.savefig('../wind_solar_hydro_emis_ternary.png', dpi=600, bbox_inches='tight')
...
# fig.subplots_adjust(left=0.075, right=0.85, wspace=0.6, hspace=0.7)
# reg_list = ['Region-wide','Argentina','Brazil','Chile','Paraguay','Uruguay']
# df1 = cap_mix[(~cap_mix.scs.str.contains('cut90'))].copy(deep=True)
# df2 = cap_mix[(cap_mix.scs.str.contains('cut90'))].copy(deep=True)
...
#     pc = ax.scatter(df1[df1.load_zone==reg_list[i]]['Solar_share'],df1[df1.load_zone==reg_list[i]]['Wind_share'],df1[df1.load_zone==reg_list[i]]['Fossil_share'],
#                     s=df1[df1.load_zone==reg_list[i]]['size'],edgecolors='k',alpha=0.7,zorder=5,clip_on=False, label='No Emissions Cut')
#     pc2 = ax.scatter(df2[df2.load_zone==reg_list[i]]['Solar_share'],df2[df2.load_zone==reg_list[i]]['Wind_share'],df2[df2.load_zone==reg_list[i]]['Fossil_share'],
#                     s=df2[df2.load_zone==reg_list[i]]['size'],edgecolors='k',alpha=0.7,zorder=5,clip_on=False,  label='90% Emissions Cut')
    
#     ax.set_tlabel("Solar",fontsize=14,labelpad=8, fontweight='bold')
#     ax.set_llabel("Wind",fontsize=14,labelpad=8, fontweight='bold')
#     ax.set_rlabel("Fossil",fontsize=14,labelpad=8, fontweight='bold')
#     ax.taxis.set_major_formatter(PercentFormatter())
#     ax.laxis.set_major_formatter(PercentFormatter())
#     ax.raxis.set_major_formatter(PercentFormatter())
#     ax.set_title(reg_list[i], fontsize=22, y=1.3)
...
# plt.show()
# #plt.savefig('../wind_solar_fossil_emis_ternary.png', dpi=600, bbox_inches='tight')
...
# fig.subplots_adjust(left=0.075, right=0.85, wspace=0.6, hspace=0.7)
# reg_list = ['Region-wide','Argentina','Brazil','Chile','Paraguay','Uruguay']
# df1 = cap_mix[(cap_mix.scs.str.contains('t1'))&(cap_mix.scs.str.contains('cut90'))].copy(deep=True)
# df2 = cap_mix[(cap_mix.scs.str.contains('tn'))&(cap_mix.scs.str.contains('cut90'))].copy(deep=True)
...
#     pc = ax.scatter(df1[df1.load_zone==reg_list[i]]['Solar_share'],df1[df1.load_zone==reg_list[i]]['Wind_share'],df1[df1.load_zone==reg_list[i]]['Hydro_share'],
#                     s=df1[df1.load_zone==reg_list[i]]['size'],edgecolors='k',alpha=0.7,zorder=5,clip_on=False, label='90% CO2 cut, Limited coord.')
#     pc2 = ax.scatter(df2[df2.load_zone==reg_list[i]]['Solar_share'],df2[df2.load_zone==reg_list[i]]['Wind_share'],df2[df2.load_zone==reg_list[i]]['Hydro_share'],
#                     s=df2[df2.load_zone==reg_list[i]]['size'],edgecolors='k',alpha=0.7,zorder=5,clip_on=False, label='90% CO2 cut, Full coord.')
...
# plt.show()
# #plt.savefig('../wind_solar_hydro_trade_ternary.png', dpi=600, bbox_inches='tight')
...
# plt.show()
# #plt.savefig('../wind_solar_fossil_trade_ternary.png', dpi=600, bbox_inches='tight')
...
# colors = ['#E80000','#770000','#E5A000','#976E00','#00DBE1','#008B8E','#6E5EE7','#1700C8','#CF44F1','#8000A0']

# for i in range(1,len(reg_list)):
    
#     pc = ax.scatter(df1[df1.load_zone==reg_list[i]]['Solar_share'],df1[df1.load_zone==reg_list[i]]['Wind_share'],df1[df1.load_zone==reg_list[i]]['Hydro_share'],
#                     c=colors[2*(i-1)],s=df1[df1.load_zone==reg_list[i]]['size'],edgecolors='k',alpha=0.7,zorder=5,clip_on=False,label=reg_list[i]+': No Emissions Cut',marker='x')
#     pc2 = ax.scatter(df2[df2.load_zone==reg_list[i]]['Solar_share'],df2[df2.load_zone==reg_list[i]]['Wind_share'],df2[df2.load_zone==reg_list[i]]['Hydro_share'],
#                     c=colors[2*(i-1)],s=df2[df2.load_zone==reg_list[i]]['size'],edgecolors=colors[2*(i-1)+1],alpha=0.7,zorder=5,clip_on=False,  label=reg_list[i]+': 90% Emissions Cut')
...
# lg = plt.legend(loc='center left', bbox_to_anchor=(1,.7), frameon=False, fontsize=14)
# for handle in lg.legendHandles:
#     handle.set_sizes([200])

# plt.show()
# #plt.savefig('../combined_wind_solar_hydro_emis_ternary.png', dpi=600, bbox_inches='tight')
...
# cap_mix['size'] = np.sqrt((cap_mix['all']/cap_mix['all'].max()))*300
# fig = plt.figure(figsize=(5, 5))
# fig.subplots_adjust(left=0.075, right=0.85, wspace=0.6, hspace=0.7)
# reg_list = ['Region-wide','Uruguay','Paraguay','Chile','Brazil','Argentina']
# df1 = cap_mix[(cap_mix.scs.str.contains('t1'))&(cap_mix.scs.str.contains('cut90'))].copy(deep=True)
# df2 = cap_mix[(cap_mix.scs.str.contains('tn'))&(cap_mix.scs.str.contains('cut90'))].copy(deep=True)
...
# colors = ['#CF44F1','#8000A0','#6E5EE7','#1700C8','#00DBE1','#008B8E','#E5A000','#976E00','#E80000','#770000']

# for i in range(1,len(reg_list)):
    
#     pc2 = ax.scatter(df2[df2.load_zone==reg_list[i]]['Solar_share'],df2[df2.load_zone==reg_list[i]]['Wind_share'],
#                       df2[df2.load_zone==reg_list[i]]['Hydro_share'],c=colors[2*(i-1)],s=df2[df2.load_zone==reg_list[i]]['size'],
#                       edgecolors='k',alpha=0.8,zorder=5,clip_on=False,label=reg_list[i]+': 90% CO2 cut, Full coord.')
    
#     pc = ax.scatter(df1[df1.load_zone==reg_list[i]]['Solar_share'],df1[df1.load_zone==reg_list[i]]['Wind_share'],
#                     df1[df1.load_zone==reg_list[i]]['Hydro_share'],c=colors[2*(i-1)],s=df1[df1.load_zone==reg_list[i]]['size'],
#                     edgecolors=colors[2*(i-1)+1],alpha=0.6,zorder=5,clip_on=False,label=reg_list[i]+': 90% CO2 cut, Limited coord.',marker='^')
...
# lg = ax.legend(handles[::-1], labels[::-1], loc='center left', bbox_to_anchor=(1,.7), frameon=False, fontsize=14)
# for handle in lg.legendHandles:
#     handle.set_sizes([200])

# plt.show()
# #plt.savefig('../combined_wind_solar_hydro_trade_ternary.png', dpi=600, bbox_inches='tight')
...
df_dispatch = pd.read_csv(datapath+'hourly_dispatch_all80.csv')
...
hyd_curt = pd.read_csv(datapath + 'scheduled_hydro_curtailment.csv')
...
solar_bio = solar_cf.merge(sol_new_biome,on=['Latitude','Longitude','project','load_zone'])
wind_bio = wind_cf.merge(wind_new_biome,on=['Latitude','Longitude','project','load_zone'])
sol_tot_bio_cap = solar_bio.groupby(['biome']).sum()[['cap_mw']]
sol_mean_bio_cf = solar_bio.drop(columns=['project','load_zone']).groupby(['biome']).mean()[['cf_1axis','cf_fixed']]
sol_mean_bio_cf['mean_cf'] = sol_mean_bio_cf[['cf_1axis','cf_fixed']].mean(axis=1)
sol_mapper = dict(zip(sol_mean_bio_cf.index,sol_mean_bio_cf.mean_cf))
wind_tot_bio_cap = wind_bio.groupby(['biome']).sum()[['cap_mw']]
wind_mean_bio_cf = wind_bio.drop(columns=['project','load_zone']).groupby(['biome']).mean()[turb_cols]
wind_mean_bio_cf['mean_cf'] = wind_mean_bio_cf[turb_cols].mean(axis=1)
wind_mapper = dict(zip(wind_mean_bio_cf.index,wind_mean_bio_cf.mean_cf))
solar_bio['biome_mean'] = solar_bio['biome'].map(sol_mapper)
wind_bio['biome_mean'] = wind_bio['biome'].map(wind_mapper)
...
# fig = plt.figure(constrained_layout=False, figsize=(4.6,7))
# gs = fig.add_gridspec(2,2, width_ratios=[1,0.6], height_ratios=[1,0.2], wspace=1.3, hspace=0)
# ylims,yt,ytl = [(-25,10)],[(0,50,100,150,200,250,300)],[('0','50','100','150','200','250','300')]
# diff_colors, diff_med_colors = ['#F49F1C','#030E4F']*4, ['#030E4F','#F49F1C']*4

# ax3 = fig.add_subplot(gs[1,:])
# g3 = plt.boxplot([13,21,30,39,47])
# for median in g3['medians']:
#     median.set_color('black')
# ax3.set_xlim(-0.06,2.3)
# ax3.set_ylim(-1,189)
# plt.sca(ax3)
# plt.axis('off')

# ax = fig.add_subplot(gs[0,1])
# g = sns.boxplot(data=cost_diffs[(cost_diffs.emis_policy==1)&(cost_diffs.variable=='tx_diff')],x='variable',y='value',hue='fixed_pv', width=0.7, ax=ax,
#                 medianprops={"linewidth": 1,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker": "o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},boxprops={"linewidth": .6,'edgecolor':'k'})
# g = sns.boxplot(data=cost_diffs[(cost_diffs.emis_policy==1)&(cost_diffs.variable=='op_diff')],x='variable',y='value',hue='fixed_pv', width=0.7, ax=ax,
#                 medianprops={"linewidth": 1,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker": "o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},boxprops={"linewidth": .6,'edgecolor':'k'})
# g = sns.boxplot(data=cost_diffs[(cost_diffs.emis_policy==1)&(cost_diffs.variable=='cap_diff')],x='variable',y='value',hue='fixed_pv', width=0.7, ax=ax,
#                 medianprops={"linewidth": 1,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker": "o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},boxprops={"linewidth": .6,'edgecolor':'k'})
...
# ax.set_ylabel('Difference in Costs when Limiting Coordination ($B)',fontsize=11, labelpad=7)
# ax.set_xticks([])
# ax.set_xticklabels([])
# ax.set_xlabel('')
# ax.set_ylim(-10,25)
# ax.annotate('Transmission\nCosts', xy=(1.1,0.10), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.annotate('Grid Operations\nCosts', xy=(1.1,0.32), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.annotate('Gen. Capacity\nCosts', xy=(1.1,0.78), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.axhline(0,color='gray',linewidth=0.8, linestyle='--')

# ax2 = fig.add_subplot(gs[0,0])
# grad_costs = pd.DataFrame(list(itertools.product(np.arange(-3,4.1,0.1),cost_diffs2.new_total_cost_y)),columns=['x','y'])
# g2 = sns.kdeplot(data=grad_costs, x="x", y="y", ax=ax2, fill=True,levels=100,cmap='new_cmap',zorder=0)
# g = sns.boxplot(data=cost_diffs2,x='exist_trd',y='new_total_cost_x',hue='fixed_pv', width=0.6, ax=ax2,
#                 medianprops={"linewidth": 1,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
...
#     line.set_color(diff_med_colors[j])
# ax2.set_xlabel('')
# ax2.set_ylim(260,370)
# ax2.set_xlim(-0.5,1.5)
# ax2.set_ylabel('Total System Costs for ' + r"$90\%\ CO_{2}\ Cut$ (\$B)",fontsize=11, labelpad=7)
# ax2.set_xticklabels([r'$\it{Full}$'+'\n'+'$\it{Coord.}$',r'$\it{Limited}$'+'\n'+'$\it{Coord.}$'],fontsize=10)
# ax2.annotate('costs under ' + r"${No\ CO_{2}\ Target}$", xy=(1.04,0.21), xycoords='axes fraction', annotation_clip=False,fontsize=7,rotation=90)

# ax2.annotate('(a)',xy=(0.125,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=13)
# ax.annotate('(b)',xy=(0.21,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=13)

# pb1 = Patch(facecolor=diff_colors[0], edgecolor=diff_med_colors[1],linewidth=0.5)
# pb2 = Patch(facecolor=diff_colors[1], edgecolor=diff_med_colors[0],linewidth=0.5)
# pb3 = Patch(facecolor='None', edgecolor='None',linewidth=0)
# ax2.legend(handles=[pb1, pb2, pb3],labels=['1-Axis Solar', 'Fixed Solar','Turbine Type'],ncol=1,bbox_to_anchor=(1.49,-0.12),frameon=False,
#           loc='center',fontsize=9,labelspacing=0.8)#,handletextpad=0.5, handlelength=1.2, columnspacing=-0.5,
# plt.show()
# #fig.savefig(figpath + 'costs_new_boxplots_v3.png', bbox_inches='tight', dpi=600)


# ### ALTERNATE VERSION WITH ARROWS ADDED ###

# plt.rcParams.update(plt.rcParamsDefault)
# font = FontProperties()
# font.set_name('Open Sans')
# plt.rcParams["font.family"] = "Arial"
# fig = plt.figure(constrained_layout=False, figsize=(5.1,7))
# gs = fig.add_gridspec(2,2, width_ratios=[1,0.85], height_ratios=[1,0.2], wspace=1, hspace=0)
# ylims,yt,ytl = [(-25,10)],[(0,50,100,150,200,250,300)],[('0','50','100','150','200','250','300')]
# diff_colors, diff_med_colors = ['#F49F1C','#030E4F']*4, ['#030E4F','#F49F1C']*4
...
# ax.annotate('Gen. Capacity\nCosts', xy=(1.1,0.78), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.axhline(0,color='gray',linewidth=0.8, linestyle='--')
# ax.arrow(-0.64,0.75,0,6,color='#C70000',width=0.043,head_width=0.14,head_length=.6)
# ax.arrow(-0.64,-0.75,0,-6,color='#C70000',width=0.043,head_width=0.14,head_length=.6)
# ax.annotate('$\it{Costlier}$', xy=(0.09,0.396), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')
# ax.annotate('$\it{Less\ Costly}$', xy=(0.09,0.184), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')
...
# ax2.annotate('costs under ' + r"${No\ CO_{2}\ Target}$", xy=(1.04,0.21), xycoords='axes fraction', annotation_clip=False,fontsize=7,rotation=90)
# ax2.annotate('(a)',xy=(0.125,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=13)
# ax.annotate('(b)',xy=(0.155,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=13)
# pb1 = Patch(facecolor=diff_colors[0], edgecolor=diff_med_colors[1],linewidth=0.5)
# pb2 = Patch(facecolor=diff_colors[1], edgecolor=diff_med_colors[0],linewidth=0.5)
# pb3 = Patch(facecolor='None', edgecolor='None',linewidth=0)
# ax2.legend(handles=[pb1, pb2, pb3],labels=['1-Axis Solar', 'Fixed Solar','Variability due to Turbine Type'],ncol=1,bbox_to_anchor=(1.035,-0.12),frameon=False,
#           loc='center left',fontsize=9,labelspacing=0.8)#,handletextpad=0.5, handlelength=1.2, columnspacing=-0.5,
# plt.show()
# #fig.savefig(figpath + 'costs_new_boxplots_arrow.png', bbox_inches='tight', dpi=600)


# ### ALTERNATE VERSION 1/26/25 collapse solar type ###
...
# gs = fig.add_gridspec(2,2, width_ratios=[1,0.6], height_ratios=[1,0.2], wspace=.97, hspace=0)
# ylims,yt,ytl = [(-25,10)],[(0,50,100,150,200,250,300)],[('0','50','100','150','200','250','300')]
...
# ax = fig.add_subplot(gs[0,1])
# g = sns.boxplot(data=cost_diffs[(cost_diffs.emis_policy==1)&(cost_diffs.variable=='tx_diff')],x='variable',y='value',width=0.5,ax=ax,
...
# g = sns.boxplot(data=cost_diffs[(cost_diffs.emis_policy==1)&(cost_diffs.variable=='op_diff')],x='variable',y='value',width=0.5,ax=ax,
#                 medianprops={"linewidth": 1.5,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker":"o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},boxprops={"linewidth":.6})
# g = sns.boxplot(data=cost_diffs[(cost_diffs.emis_policy==1)&(cost_diffs.variable=='cap_diff')],x='variable',y='value',width=0.5,ax=ax,
#                 medianprops={"linewidth": 1.5,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker":"o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},boxprops={"linewidth":.6})
# ax.legend([],frameon=False)
# boxes = ax.findobj(matplotlib.patches.PathPatch)
...
# for j, line in enumerate(median_lines):
#     line.set_color(diff_med_colors[j])
# ax.set_ylabel(r'Benefits of $\it{Full\ Coordination}$ (\$B)',fontsize=11, labelpad=-1)
# ax.set_xticks([])
# ax.set_xticklabels([])
# ax.set_xlabel('')
# ax.set_ylim(-10,25)
# ax.set_xlim(-1,0.5)
# ax.annotate('Transmission\nExpansion', xy=(1.1,0.10), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.annotate('Grid Operations', xy=(1.1,0.34), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.annotate('New Capacity', xy=(1.1,0.8), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.axhline(0,color='gray',linewidth=0.8, linestyle='--')
# ax.arrow(-0.54,0.75,0,6,color='#C70000',width=0.043,head_width=0.14,head_length=.6)
# ax.arrow(-0.54,-0.75,0,-6,color='#C70000',width=0.043,head_width=0.14,head_length=.6)
# ax.annotate('$\it{Cost\ Savings}$', xy=(0.09,0.396), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')
# ax.annotate('$\it{Expenditures}$', xy=(0.09,0.184), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')

# ax2 = fig.add_subplot(gs[0,0])
# grad_costs = pd.DataFrame(list(itertools.product(np.arange(-3,4.1,0.1),cost_diffs2.new_total_cost_y)),columns=['x','y'])
# g2 = sns.kdeplot(data=grad_costs, x="x", y="y", ax=ax2, fill=True,levels=100,cmap=new_cmap,zorder=0)
# g = sns.boxplot(data=cost_diffs2,x='exist_trd',y='new_total_cost_x', width=0.55, ax=ax2,
#                 medianprops={"linewidth": 1.5,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker": "o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},
#                 boxprops={"linewidth": .6,'edgecolor':'k','facecolor':'white'})
# ax2.legend([],frameon=False)
...
# ax2.set_ylim(260,370)
# ax2.set_xlim(-0.5,1.5)
# ax2.set_ylabel('Total System Costs ($B)',fontsize=11, labelpad=7)
# ax2.set_xticklabels([r'$\it{Full}$'+'\n'+'$\it{Coord.}$',r'$\it{Limited}$'+'\n'+'$\it{Coord.}$'],fontsize=10)
# ax2.annotate(r' $Reference$ costs'+'\n' + r"${(No\ CO_{2}\ Target)}$",xy=(1.036,0.265),xycoords='axes fraction',annotation_clip=False,fontsize=7,rotation=90)
# ax2.annotate('(a)',xy=(0.125,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=13)
# ax.annotate('(b)',xy=(0.21,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=13)
# plt.show()
# #fig.savefig(figpath + 'costs_boxplots_arrow.png', bbox_inches='tight', dpi=600)
...
# ### ALTERNATE VERSION 2/17/25 plot left panel as % increase from no co2 target ###
# plt.rcParams.update(plt.rcParamsDefault)
...
# font.set_name('Open Sans')
# plt.rcParams["font.family"] = "Arial"
# fig = plt.figure(constrained_layout=False, figsize=(5,7.2))
# gs = fig.add_gridspec(2,2, width_ratios=[1,0.85], height_ratios=[1,0.2], wspace=1.6, hspace=0)
# ylims,yt,ytl = [(-25,10)],[(0,50,100,150,200,250,300)],[('0','50','100','150','200','250','300')]
# diff_colors2, diff_med_colors2 = ['#429DED','#E4B321'], ['#0074DA','#AD8200']
# diff_colors, diff_med_colors = ['#030E4F','#030E4F','#030E4F'], ['#408298','#408298','#408298']
...
#                 medianprops={"linewidth": 1.5,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker":"o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},boxprops={"linewidth":.6})
# g = sns.boxplot(data=cd_opp[(cd_opp.emis_policy==1)&(cd_opp.variable=='op_diff')],x='variable',y='value',width=0.5,ax=ax,
#                 medianprops={"linewidth": 1.5,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker":"o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},boxprops={"linewidth":.6})
# g = sns.boxplot(data=cd_opp[(cd_opp.emis_policy==1)&(cd_opp.variable=='cap_diff')],x='variable',y='value',width=0.5,ax=ax,
#                 medianprops={"linewidth": 1.5,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                 flierprops={"marker":"o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},boxprops={"linewidth":.6})
# ax.legend([],frameon=False)
# boxes = ax.findobj(matplotlib.patches.PathPatch)
...
# for j, line in enumerate(median_lines):
#     line.set_color(diff_med_colors[j])
# ax.set_ylabel('Benefits of coordination under '+r'$Mitigation$',fontsize=16, labelpad=34)
# ax.set_xticks([])
# ax.set_xticklabels([])
# ax.set_xlabel('')
# ax.set_ylim(-25,15)
# ax.set_xlim(-1.2,0.5)
# vals1 = ax.get_yticks()
# ax.set_yticklabels(['-$25B','-$20B','-$15B','-$10B','-$5B','$0','$5B','$10B','$15B'], fontsize=11)
# ax.annotate('Transmission\nexpansion', xy=(1.06,0.73), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.annotate('Grid operations\n(includes fuel costs)', xy=(1.06,0.538), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.annotate('New capacity', xy=(1.06,0.15), xycoords='axes fraction', annotation_clip=False,fontsize=11)
# ax.axhline(0,color='gray',linewidth=0.8, linestyle='--')
# ax.arrow(-0.64,0.75,0,6,color='#C70000',width=0.036,head_width=0.14,head_length=.6)
# ax.arrow(-0.64,-0.75,0,-6,color='#C70000',width=0.036,head_width=0.14,head_length=.6)
# ax.annotate('$\it{Cost\ savings}$', xy=(0.11,0.523), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')
# ax.annotate('$\it{Expenditures}$', xy=(0.11,0.727), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')
# # ax.annotate('$\it{(difference\ between\ Full\ and\ Limited\ Coordination)}$',xy=(-0.92,0.5),xycoords='axes fraction',annotation_clip=False,fontsize=10,rotation=90,ha='left',va='center')
# plt.annotate('(difference between '+r'$Full$'+' and '+r'$Limited\ Coordination$'+')',xy=(-0.92,0.5),xycoords='axes fraction',annotation_clip=False,fontsize=10,rotation=90,ha='left',va='center')
...
# ax2.set_xlim(-0.5,1.5)
# ax2.set_ylabel('Cost premium of '+r'$Mitigation$',fontsize=16, labelpad=16)
# ax2.set_xticklabels([r'$\it{Full}$'+'\n'+'$\it{Coord.}$',r'$\it{Limited}$'+'\n'+'$\it{Coord.}$'],fontsize=10)
# ax2.annotate('(a)',xy=(0.17,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=13)
# ax.annotate('(b)',xy=(0.21,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=13)
# plt.show()
# #fig.savefig(figpath + 'costs_boxplots_arrow.png', bbox_inches='tight', dpi=600)
...
# ### ALTERNATE VERSION 3/21/25 add new middle panel to show net cost savings ###
# plt.rcParams.update(plt.rcParamsDefault)
# plt.rcParams['mathtext.fontset'] = 'custom'
# plt.rcParams['mathtext.rm'] = 'Bitstream Vera Sans'
# plt.rcParams['mathtext.it'] = 'Bitstream Vera Sans:italic'
...
# for j, line in enumerate(median_lines):
#     line.set_color(diff_med_colors[j])
# ax.set_ylabel('Benefits of ' + r'$Coordination$' + ' under ' + r'$Mitigation$',fontsize=16, labelpad=34)
# ax.set_xticks([])
# ax.set_xticklabels([])
# ax.set_xlabel('')
# ax.set_ylim(-25,13)
# ax.set_xlim(-1.2,0.47)
# vals1 = ax.get_yticks()
...
# ax.arrow(-0.64,0.75,0,6,color='#C70000',width=0.036,head_width=0.14,head_length=.6)
# ax.arrow(-0.64,-0.75,0,-6,color='#C70000',width=0.036,head_width=0.14,head_length=.6)
# ax.annotate('$\it{Cost\ savings}$', xy=(0.11,0.56), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')
# ax.annotate('$\it{Expenditures}$', xy=(0.11,0.764), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')
# plt.annotate('(difference between '+r'$Full$'+' and '+r'$Limited\ Coordination$'+')',xy=(-0.98,0.5),xycoords='axes fraction',annotation_clip=False,fontsize=10,rotation=90,ha='left',va='center')
...
# ax2.set_yticklabels(['{:.1%}'.format(x) for x in vals], fontsize=11)
# ax2.set_xlim(-0.5,1.5)
# ax2.set_ylabel('Cost premium of '+r'$Mitigation$',fontsize=16, labelpad=13)
# ax2.set_xticklabels([r'$\it{Full}$'+'\n'+'$\it{Coord.}$',r'$\it{Limited}$'+'\n'+'$\it{Coord.}$'],fontsize=10)
...
# # ax3.arrow(0,-0.75,0,-6,color='#C70000',width=0.036,head_width=0.14,head_length=.6)
# # ax3.annotate('$\it{Cost\ savings}$', xy=(0.105,0.56), xycoords='axes fraction', annotation_clip=False,fontsize=8,rotation=90,ha='left',va='center')
# ax3.set_xlim(-0.6,1.6)
# ax3.set_ylabel('Net Cost Savings of '+r'$Coordination$',fontsize=16, labelpad=13)
# ax3.set_xticklabels([r'$\ \ \ \ \ \ \ \ \ \ \ \ Mitigation$' + '  ' + r'$\ Reference$',''],fontsize=10)
# # ax3.set_xticklabels([r'$Mitigation$',r'$\ Ref.$'],fontsize=10)
# ax3.tick_params(axis='x', which='major', pad=8)

# ax2.annotate('(a)',xy=(0.21,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=15)
# ax.annotate('(c)',xy=(0.23,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=15)
# ax3.annotate('(b)',xy=(0.21,0.964), xycoords='axes fraction',ha='center',va='center',fontweight='bold',fontsize=15)
# plt.show()
# #fig.savefig(figpath + 'costs_boxplots_arrow.png', bbox_inches='tight', dpi=600)


#%% MAP AND VRE PREFERENCE (GEOPANDAS NEEDED)
...
# ar_tx = geopandas.read_file(datapath + 'transmission_georef/Argentina/lineas_de_energia_AT030.shp')
# uy_tx = geopandas.read_file(datapath + 'transmission_georef/Uruguay/Transmission.shp')
# py_tx = geopandas.read_file(datapath + 'transmission_georef/Paraguay/Transmission_Para.shp')
# br_tx = geopandas.read_file(datapath + 'transmission_georef/Brazil/BRA_ags_Download_Webmap_EPE/zipfolder/Linhas_de_Transmissão___Base_Existente.shp')
# cl_tx = geopandas.read_file(datapath + 'transmission_georef/Chile/Transmission_Chile.shp')
# cl_tx = cl_tx.to_crs(epsg=4326)
# py_tx = py_tx.to_crs(epsg=4326)
# uy_tx = uy_tx.to_crs(epsg=4326)
# br_tx = br_tx.to_crs('epsg:4326')
...
# gs1 = gs[0].subgridspec(1,6, width_ratios=[1,0.001,1,0.001,1,0.15], wspace=.09)
# gs2 = gs[1].subgridspec(1,2, width_ratios=[1,0.1], wspace=0)
# axp = fig.add_subplot(gs2[0])
# axm1,axm2,axm3 = fig.add_subplot(gs1[2]),fig.add_subplot(gs1[4]),fig.add_subplot(gs1[0])
...
# countries.plot(ax=axm2, color='w',zorder=0,edgecolor='k',linewidth=.15)
# ar_tx['lengths'] = ar_tx['geometry'].length
# tx_color = '#6F0606'
# ar_tx[ar_tx.lengths>=0.1].plot(ax=axm2, color=tx_color,zorder=1,linewidth=.2)
# ar_tx[ar_tx.lengths<0.1].plot(ax=axm2, color=tx_color,zorder=1,linewidth=.04)
# br_tx.plot(ax=axm2, color=tx_color,zorder=1,linewidth=.2)
# cl_tx.plot(ax=axm2, color=tx_color,zorder=1,linewidth=.2)
# py_tx.plot(ax=axm2, color=tx_color,zorder=1,linewidth=.2)
# uy_tx.plot(ax=axm2, color=tx_color,zorder=1,linewidth=.2)
# uy_tx.plot(ax=axm3, color="none",zorder=0,linewidth=.2)
# uy_tx.plot(ax=axm1, color="none",zorder=0,linewidth=.2)
# plt.sca(axm2)
# plt.xlim(-77,-30)
# plt.ylim(-57,6)
...
# axp.set_yticklabels([r"${No\ CO_{2}\ Target}$"+' +\n'+'$\it{Limited\ Coordination}$',r"${No\ CO_{2}\ Target}$"+' +\n'+'$\it{Full\ Coordination}$',
#                       r"$90\%\ CO_{2}\ Cut$"+' +\n'+'$\it{Limited\ Coordination}$',r"$90\%\ CO_{2}\ Cut$"+' +\n'+'$\it{Full\ Coordination}$'],fontsize=10.5)
...
# axm2.annotate('(c)',(-81,-1),ha='left',fontsize=11,fontweight='bold', annotation_clip=False)
# axm3.annotate('(a)',(-81,-1),ha='left',fontsize=11,fontweight='bold', annotation_clip=False)
# axp.annotate('(d)',(0.01,-0.41),ha='left',fontsize=11,fontweight='bold')
...
# plt.show()
# #plt.savefig(figpath + 'vre_preference_plus_map.png', bbox_inches='tight',dpi=1200)

#%% MAP AND PARALLEL AXIS (GEOPANDAS, BIOMES) (SI)

# pa_vre = pd.read_csv(datapath + 'parallel_axis_vre_df.csv',index_col='scenario')
# df_f = pa_vre[pa_vre.fixed_pv==True]
# df_s = pa_vre[pa_vre.fixed_pv==False]
# pa_vre = pa_vre[['pol']+pa_cols]
# df_f = df_f[['pol']+pa_cols]
# df_s = df_s[['pol']+pa_cols]

# max_value, min_value = round(pa_vre[pa_cols].max().max(),-3)+1000, 0
# labels = [i.split(' ', 1)[1] for i in pa_cols]
# # adjust for line splits
# for i in range(len(labels)):
#     if labels[i] == 'Gran Chaco':
#         labels[i] = 'Gran\nChaco'
#     if labels[i] == 'Atacama/Andes':
#         labels[i] = 'Atacama/\nAndes'
#     if labels[i] == 'Monte Desert':
#         labels[i] = 'Monte\nDesert'
...
# gs = fig.add_gridspec(2,1, height_ratios=[0.6,1], hspace=.2)
# gs1 = gs[0].subgridspec(1,5, width_ratios=[1,0.01,1,1,0.2], wspace=.15)
# axp = fig.add_subplot(gs[1])
# axm1,axm2,axm3 = fig.add_subplot(gs1[0]),fig.add_subplot(gs1[3]),fig.add_subplot(gs1[2])
# ### biome map boundaries ###
# hue_orders,hue_orderw=['Pampas','Atacama/Andes','Gran Chaco','Caatinga','Cerrado','Monte Desert'],['Pampas','Patagonia','Gran Chaco','Caatinga','Coastal']
# color_s,color_w = ['#88CCEE','#FF9000','#50e991','#e60049','#9b19f5','#924400'],['#88CCEE','#e6d800','#50e991','#e60049','#0bb4ff']
# countries.plot(ax=axm1, color='none',zorder=0,edgecolor='k',linewidth=.15)
# g1 = sns.scatterplot(x='Longitude',y='Latitude',hue='biome',data=wind_bio,ax=axm1,marker=path,palette=color_w,s=12,edgecolor='none',hue_order = hue_orderw)
# plt.sca(axm1)
# plt.xlim(-77,-30)
# plt.ylim(-57,6)
# ms = 6
# custom_lines = [Line2D([], [], color=color_w[0], marker='s', linestyle='none', markersize=ms, label=hue_orderw[0]),
#                 Line2D([], [], color=color_w[1], marker='s', linestyle='none', markersize=ms, label=hue_orderw[1]),
#                 Line2D([], [], color=color_w[2], marker='s', linestyle='none', markersize=ms, label=hue_orderw[2]),
#                 Line2D([], [], color=color_w[3], marker='s', linestyle='none', markersize=ms, label=hue_orderw[3]),
#                 Line2D([], [], color=color_w[4], marker='s', linestyle='none', markersize=ms, label=hue_orderw[4])]
# plt.legend(handles=custom_lines,loc='lower right',bbox_to_anchor=(1.25,-0.03),frameon=False,labelspacing=0.6,handletextpad=0,fontsize="8")
# plt.title('Wind Project Biomes', fontsize=10.5)
# countries.plot(ax=axm1, color='none',zorder=10,edgecolor='k',linewidth=.15,linestyle='--',alpha=0.5)
# plt.axis('off')

# countries.plot(ax=axm2, color='none',zorder=0,edgecolor='k',linewidth=.15)
# g2 = sns.scatterplot(x='Longitude',y='Latitude',hue='biome',data=solar_bio,ax=axm2,marker=path,palette=color_s,s=12,edgecolor='none',hue_order = hue_orders)
# plt.sca(axm2)
# plt.xlim(-77,-30)
# plt.ylim(-57,6)
# custom_lines = [Line2D([], [], color=color_s[0], marker='s', linestyle='none', markersize=ms, label=hue_orders[0]),
#                 Line2D([], [], color=color_s[1], marker='s', linestyle='none', markersize=ms, label=hue_orders[1]),
#                 Line2D([], [], color=color_s[2], marker='s', linestyle='none', markersize=ms, label=hue_orders[2]),
#                 Line2D([], [], color=color_s[3], marker='s', linestyle='none', markersize=ms, label=hue_orders[3]),
#                 Line2D([], [], color=color_s[4], marker='s', linestyle='none', markersize=ms, label=hue_orders[4]),
#                 Line2D([], [], color=color_s[5], marker='s', linestyle='none', markersize=ms, label=hue_orders[5])]
# plt.legend(handles=custom_lines,loc='lower right',bbox_to_anchor=(1.43,-0.03),frameon=False,labelspacing=0.6,handletextpad=0,fontsize="8")
# plt.title('Solar PV Project Biomes', fontsize=10.5)
# countries.plot(ax=axm2, color='none',zorder=10,edgecolor='k',linewidth=.15,linestyle='--',alpha=0.5)
# plt.axis('off')

# countries.plot(ax=axm3, color='none',zorder=0,edgecolor='k',linewidth=.15)
# ar_tx['lengths'] = ar_tx['geometry'].length
# tx_color = '#6F0606'
# ar_tx[ar_tx.lengths>=0.1].plot(ax=axm3, color=tx_color,zorder=1,linewidth=.2)
# ar_tx[ar_tx.lengths<0.1].plot(ax=axm3, color=tx_color,zorder=1,linewidth=.04)
# br_tx.plot(ax=axm3, color=tx_color,zorder=1,linewidth=.2)
# cl_tx.plot(ax=axm3, color=tx_color,zorder=1,linewidth=.2)
# py_tx.plot(ax=axm3, color=tx_color,zorder=1,linewidth=.2)
# uy_tx.plot(ax=axm3, color=tx_color,zorder=1,linewidth=.2)

# plt.sca(axm3)
# plt.xlim(-77,-30)
# plt.ylim(-57,6)
# plt.title('Existing Transmission', fontsize=10.5)
# plt.axis('off')
# ### parallel axis in bottom panel ###
# parallel_coordinates(pa_vre.sort_values(by=pa_vre.columns[0]),'pol',ax=axp,color=color4,alpha=0,axvlines_kwds={'color':'k','alpha':0.5,'linewidth':0.5})
# parallel_coordinates(df_s.sort_values(by=df_s.columns[0],ascending=False),'pol',ax=axp, color=color4, alpha=0.75,linestyle='-',axvlines=False,linewidth=1.2)
# parallel_coordinates(df_f.sort_values(by=df_f.columns[0],ascending=False),'pol',ax=axp, color=color4, alpha=0.75,linestyle=':',axvlines=False,linewidth=1.7)
# axp.set_xticklabels(labels, fontsize=7.5, fontweight='bold')
# axp.set_ylim(0,160000)
# axp.set_yticks([0, 20000, 40000, 60000, 80000, 100000, 120000, 140000, 160000])
# axp.set_yticklabels(['0', '20', '40', '60', '80', '100', '120', '140', '160'])
# axp.set_ylabel('Total New Selected Capacity (GW)',labelpad=8)
# axp.grid(False)
# axp.legend('',frameon=False)
# lw, location_legend = 2.6, (0.5,-0.2)
# custom_lines = [Line2D([0], [0], color=color4[3], lw=lw, alpha=0.8),
#                 Line2D([0], [0], color=color4[2], lw=lw, alpha=0.8),
#                 Line2D([0], [0], color=color4[1], lw=lw, alpha=0.8),
#                 Line2D([0], [0], color=color4[0], lw=lw, alpha=0.8),
#                 Line2D([0], [0], color='k', lw=lw, alpha=0.9),
#                 Line2D([0], [0], color='k', lw=lw-0.1, alpha=0.9, linestyle=':')]
# plt.sca(axp)
# l1=plt.legend(custom_lines,['Fixed Solar','1-axis Solar',r"${No\ CO_{2}\ Target}$"+' + '+'$\it{Limited\ Coordination}$',r"${No\ CO_{2}\ Target}$"+' + '+'$\it{Full\ Coordination}$',
#                             r"$90\%\ CO_{2}\ Cut$"+' + '+'$\it{Limited\ Coordination}$',r"$90\%\ CO_{2}\ Cut$"+' + '+'$\it{Full\ Coordination}$'][::-1],
#                             handlelength=1.7,loc='center',bbox_to_anchor=location_legend,fontsize=9.5,frameon=False,ncol=3)
# axp.add_artist(l1)
# axp.axvline(x=4.5, ymin=-0.11, ymax=1.075,color='gray', linewidth=1, zorder=0, linestyle='--',clip_on=False)
# axp.axhline(y=0,color='k', linewidth=0.8, zorder=0)
# axp.annotate('Wind',(2.25,165000),ha='center', annotation_clip=False,fontsize=15)
# axp.annotate('Solar PV',(7.25,165000),ha='center', annotation_clip=False,fontsize=15)
# axm1.annotate('(a)',(-81,0),ha='left',fontsize=11,fontweight='bold', annotation_clip=False)
# axm2.annotate('(c)',(-81,0),ha='left',fontsize=11,fontweight='bold', annotation_clip=False)
# axm3.annotate('(b)',(-81,0),ha='left',fontsize=11,fontweight='bold', annotation_clip=False)
# axp.annotate('(d)',(0.15,149500),ha='left',fontsize=11,fontweight='bold')

# plt.show()
# #plt.savefig(figpath + 'SI/parallel_axis_vre_map_combined_tx.png', bbox_extra_artists=(l1,), bbox_inches='tight',dpi=1100)
...
# plt.show()
# #plt.savefig(figpath + 'vre_map_wind_cf.png', bbox_inches='tight',dpi=1000)
...
# plt.show()
# #plt.savefig(figpath + 'vre_map_solar_cf.png', bbox_inches='tight',dpi=1000)

#%% TRADE BOXPLOTS PLUS BARPLOTS

# #new_med_c = ['#CA1658','#FF96BC','#0074DA','#84C5FF']*6
# nsns_med_c = ['#CA1658','#CA1658','#CA1658','#CA1658','#CA1658','#CA1658',
#              '#FF96BC','#FF96BC','#FF96BC','#FF96BC','#FF96BC','#FF96BC',
#              '#0074DA','#0074DA','#0074DA','#0074DA','#0074DA','#0074DA',
#              '#84C5FF','#84C5FF','#84C5FF','#84C5FF','#84C5FF','#84C5FF']
# plt.rcParams.update(plt.rcParamsDefault)
# font = FontProperties()
# font.set_name('Open Sans')
# plt.rcParams["font.family"] = "Arial"
...
# g = sns.barplot(x='load_zone',y='trade',data=tot_ex[tot_ex.group.isin(['001','101','000','100'])],ax=ax1,saturation=1,hue='group',
#             palette=palette,estimator=np.mean,errorbar=('pi',100),capsize=0,order=mercosur,err_kws={'linewidth':1.6},hue_order=['000','001','100','101'])
# ax1.set_xticklabels(mercosur, fontsize=8, fontweight='bold')
...
# g = sns.barplot(x='load_zone',y='trade',data=tot_im[tot_im.group.isin(['001','101','000','100'])],ax=ax2,saturation=1,hue='group',
#             palette=palette,estimator=np.mean,errorbar=('pi',100),capsize=0,order=mercosur,err_kws={'linewidth':1.6},hue_order=['000','001','100','101'])
# ax2.set_xticklabels([])
# ax2.set_ylim(0,310)
...
# g = sns.boxplot(data=all_trd_tot[(all_trd_tot.exist_trd==0)].sort_values(by='lz_to'),x='connection',y='net_flow_twh',hue='group',ax=ax3,
#                 width=0.7,medianprops={"linewidth":.5,'color':'k','label': '_median_','solid_capstyle':'butt'},boxprops={"linewidth":0.3,'edgecolor':'k'},
#                 flierprops={"marker": "o",'markerfacecolor':'none','markeredgecolor':'k','markersize':3},palette=palette,saturation=1,zorder=10,
#                 hue_order=['000','001','100','101'],order=corridorder)
# box_patches = [patch for patch in ax3.patches if type(patch) == matplotlib.patches.PathPatch]
# if len(box_patches) == 0:
#     box_patches = ax3.artists
# num_patches = len(box_patches)
...
#     line.set_color(nsns_med_c[i])
# ax3.set_ylim(0,250)
# ax3.set_ylabel('Cumulative Electricity Trade (TWh)',labelpad=10,fontsize=13)
# ax3.set_xticklabels(corridorder_lab,fontsize=9, fontweight='bold')
...
# ax3.legend([],frameon=False)
# legend_elements = [Line2D([0], [0],label=r"${No\ CO_{2}\ Target}$" + ' + 1-Axis Solar', color=palette['000'], lw=7),
#                     Line2D([0], [0],label=r"${No\ CO_{2}\ Target}$" + ' + Fixed Solar', color=palette['001'], lw=7),
#                     Line2D([0], [0],label=r"${90\%\ CO_{2}\ Cut}$" + ' + 1-Axis Solar', color=palette['100'], lw=7),
#                     Line2D([0], [0],label=r"${90\%\ CO_{2}\ Cut}$" + ' + Fixed Solar', color=palette['101'], lw=7),
#                     Line2D([0], [0],label=r"$\it{Limited\ Coord.}$" + 'scenario range', color=lim_coord_col, alpha=0.5,lw=5)]
# ax3.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(0,1.01), frameon=False, fontsize=7.4,handlelength=0.85,labelspacing=0.4)
...
# plt.show()
# #plt.savefig(figpath + 'trade_box_plus_bar_.png', bbox_inches='tight',dpi=600)



# ### v2 (1/28/25) collapse solar, adjust colors
...
# ax3.set_xlim(-0.5,5.5)
# ax3.legend([],frameon=False)
# legend_elements = [Line2D([0], [0],label=r"${Reference\ (No\ CO_{2}\ Target)}$", color=nopv_pal[0], lw=7),
#                     Line2D([0], [0],label=r"${Mitigation\ (90\%\ CO_{2}\ Cut)}$", color=nopv_pal[1], lw=7),
#                     Line2D([0], [0],label=r"$\it{Limited\ Coordination}$", color=lim_coord_col, alpha=0.5,lw=5)]
...
# font.set_name('Open Sans')
# plt.rcParams["font.family"] = "Arial"
# fig = plt.figure(constrained_layout=False, figsize = (9.15,5.3))
# gs = fig.add_gridspec(2,2, height_ratios=[1,1], hspace=0.12, width_ratios=[1.3,1], wspace=0.22)
# ax1,ax2,ax3 = fig.add_subplot(gs[0,1]),fig.add_subplot(gs[1,1]),fig.add_subplot(gs[:,0])
# g = sns.barplot(x='load_zone',y='trade',data=tot_ex[tot_ex.group.isin(['001','101','000','100'])],ax=ax1,saturation=1,hue='group',
#             palette=palette,estimator=np.mean,errorbar=('pi',100),capsize=0,order=mercosur,errwidth=1.6,hue_order=['000','001','100','101'])
# ax1.set_xticklabels(mercosur, fontsize=9)
# ax1.tick_params(axis='x', which='major', pad=1.1)
# ax1.tick_params(labelbottom=True, labeltop=False, labelleft=True, labelright=False, bottom=False, top=False, left=True, right=False)
# ax1.set_ylim(0,310)
# ax1.set_yticks([0,100,200,300])
# ax1.set_yticklabels(['0','100','200','300'], fontsize=10)
# ax1.set_ylabel('Export from (TWh)',labelpad=7, fontsize=11)
# ax1.legend([],frameon=False)
# g = sns.barplot(x='load_zone',y='trade',data=tot_im[tot_im.group.isin(['001','101','000','100'])],ax=ax2,saturation=1,hue='group',
#             palette=palette,estimator=np.mean,errorbar=('pi',100),capsize=0,order=mercosur,errwidth=1.6,hue_order=['000','001','100','101'])
# ax2.set_xticklabels([])
# ax2.set_ylim(0,310)
# ax2.set_yticks([0,100,200,300])
# ax2.set_yticklabels(['0','100','200','300'], fontsize=10)
# ax2.set_ylabel('Import to (TWh)',labelpad=7, fontsize=11)
# ax1.set_xlabel('')
# ax2.set_xlabel('')
# ax2.tick_params(labelbottom=False, labeltop=False, labelleft=True, labelright=False, bottom=False, top=False, left=True, right=False)
# ax2.legend([],frameon=False)
...
# corridorder = ['Argentina-Paraguay','Argentina-Uruguay','Brazil-Uruguay','Argentina-Brazil','Argentina-Chile','Brazil-Paraguay']
# corridorder_lab=['AR {} PY'.format(u"\u2212"),'AR {} UY'.format(u"\u2212"),'AR {} BR'.format(u"\u2212"),
#                   'AR {} CL'.format(u"\u2212"),'BR {} PY'.format(u"\u2212"),'BR {} UY'.format(u"\u2212")]
# g = sns.boxplot(data=all_trd_tot[(all_trd_tot.exist_trd==0)].sort_values(by='lz_to'),x='connection',y='net_flow_twh',hue='group',ax=ax3,
#                 width=0.7,medianprops={"linewidth":.5,'color':'k','label': '_median_','solid_capstyle':'butt'},boxprops={"linewidth":0.3,'edgecolor':'k'},
#                 flierprops={"marker": "o",'markerfacecolor':'none','markeredgecolor':'k','markersize':3},palette=palette,saturation=1,zorder=10,
#                 hue_order=['000','001','100','101'],order=corridorder)
...
# ax3.set_yticklabels(['0','50','100','150','200','250'], fontsize=10)
# ax3.set_ylabel('Cumulative Electricity Trade (TWh)',labelpad=8,fontsize=11)
# ax3.set_xticklabels(corridorder_lab,fontsize=9, fontweight='bold')
# ax3.set_xlabel('Interconnection',fontsize=10)
# ax3.tick_params(axis='x', which='major', pad=1.5, length=2.5)
# ax3.tick_params(labelbottom=True, labeltop=False, labelleft=True, labelright=False, bottom=True, top=False, left=True, right=False)
# lim_coord_col = '#FFB200'
# plt.sca(ax3)
# plt.fill_between((0.6,1.4),y1=10,y2=12.2,zorder=1,color=lim_coord_col,alpha=0.5,edgecolor="None")
# plt.fill_between((2.6,3.4),y1=2.9,y2=3.8,zorder=1,color=lim_coord_col,alpha=0.5,edgecolor="None")
# plt.fill_between((1.6,2.4),y1=13.4,y2=16.2,zorder=1,color=lim_coord_col,alpha=0.5,edgecolor="None")
# ax3.set_xlim(-0.5,5.5)
# ax3.legend([],frameon=False)
# legend_elements = [Line2D([0], [0],label=r"${No\ CO_{2}\ Target}$" + ' + 1-Axis Solar', color=palette['000'], lw=7.5),
#                     Line2D([0], [0],label=r"${No\ CO_{2}\ Target}$" + ' + Fixed Solar', color=palette['001'], lw=7.5),
#                     Line2D([0], [0],label=r"${90\%\ CO_{2}\ Cut}$" + ' + 1-Axis Solar', color=palette['100'], lw=7.5),
#                     Line2D([0], [0],label=r"${90\%\ CO_{2}\ Cut}$" + ' + Fixed Solar', color=palette['101'], lw=7.5),
#                     Line2D([0],[0],label=r"$\it{Limited\ Coord.}$" + 'scenario range',color=lim_coord_col,alpha=0.5,lw=6.5)]
# ax3.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(0,1.017), frameon=False, fontsize=9,handlelength=0.85,labelspacing=0.35)

# ax3.annotate('AR:  Argentina\nBR:  Brazil\nCL:  Chile\nPY:  Paraguay\nUY:  Uruguay',xy=(0.018,0.767),xycoords='axes fraction',ha='left',va='top',fontsize=8.5)
# ax3.annotate('(a)',(0.013,0.5),xycoords='axes fraction',fontsize=14,fontweight='bold',ha='left',va='center')
# ax1.annotate('(b)',(0.9,0.91),xycoords='axes fraction',fontsize=14,fontweight='bold',ha='left',va='center')
# ax2.annotate('(c)',(0.9,0.09),xycoords='axes fraction',fontsize=14,fontweight='bold',ha='left',va='center')
# plt.show()
# #plt.savefig(figpath + 'trade_box_plus_bar_poster.png', bbox_inches='tight',dpi=600,facecolor='None')

#%% BATTERY + CURTAILMENT BOXPLOTS (SI)

# plt.rcParams.update(plt.rcParamsDefault)
# font = FontProperties()
# font.set_name('Open Sans')
# plt.rcParams["font.family"] = "Arial"
# fig = plt.figure(constrained_layout=False, figsize = (6.3,3.7))
# gs = fig.add_gridspec(1,2, width_ratios=[1,1], wspace=0.48)
# ylims,yt=[(-0.05,20.3),(-0.9,358),(0,1700)],[(0,5,10,15,20),(0,50,100,150,200,250,300,350),(0,500,1000,1500)]
# ytl=[('0','5','10','15','20'),('0','50','100','150','200','250','300','350'),('0','500','1000','1500')]
# decarb_labels = ['Battery Storage Requirement $(GW)$','Cumulative VRE Curtailment $(TWh)$']
# panel_labels = ['(a)','(b)']
# for i in range(len(decarb_cols)-1):
#     ax = fig.add_subplot(gs[i])
#     g = sns.boxplot(data=decarb[decarb.load_zone=='Region-wide'],x='pol',y=decarb_cols[i],hue='fixed_pv', width=0.75,
#                     medianprops={"linewidth": .7,'color':'k','label':'_median_','solid_capstyle':'butt'},saturation=1,
#                     flierprops={"marker": "o",'markerfacecolor':'none','markeredgecolor':'k','markersize':2},
#                     boxprops={"linewidth": 0.3,'edgecolor':'k'})#,whiskerprops={"linewidth": .7,'color':'k'},capprops={"linewidth": .7,'color':'k'})
#     ax.legend([],frameon=False)
#     boxes = ax.findobj(matplotlib.patches.PathPatch)
#     for color, box in zip(my_colours, boxes):
#         box.set_facecolor(color)
...
#     median_lines = [line for line in ax.get_lines() if line.get_label() == '_median_']
#     for j, line in enumerate(median_lines):
#         line.set_color(med_colours[j])
#     ax.set_ylabel(decarb_labels[i],fontsize=11,labelpad=8+i)
#     ax.set_xticklabels([r"${No\ CO_{2}\ Target}$"+'\n'+'$\it{Limited\ Coord.}$',r"${No\ CO_{2}\ Target}$"+'\n'+'$\it{Full\ Coord.}$',
#                         r"$90\%\ CO_{2}\ Cut$"+'\n'+'$\it{Limited\ Coord.}$',r"$90\%\ CO_{2}\ Cut$"+'\n'+'$\it{Full\ Coord.}$'],fontsize=9.5,rotation=90)
#     ax.set_xlabel('')
#     ax.set_ylim(ylims[i])
#     ax.set_yticks(yt[i])
#     ax.set_yticklabels(ytl[i])
#     ax.annotate(panel_labels[i],xy=(0.09,0.95),xycoords='axes fraction',ha='center',va='center',fontsize=12,fontweight='bold')
...
# l1 = plt.legend(handles=[pa1, pb1, pa2, pb2, pa3, pb3, pa4, pb4],
#           labels=['', '', '', '', '', '', '1-Axis Solar', 'Fixed Solar'],
#           ncol=4, handletextpad=0.5, handlelength=1.3, columnspacing=-0.5,
#           loc='center', fontsize=11,frameon=False,bbox_to_anchor=(-0.24,-0.53))
# ax.add_artist(l1)

# plt.show()
# #fig.savefig(figpath + 'decarbonization_v4.png', bbox_inches='tight', dpi=600)

#%% NEW CAPACITY REQUIREMENTS BOXPLOTS (SI)
...
#         else:
#             ax.set_xticklabels([r"${No\ CO_{2}\ Target}$"+'\n'+'$\it{Limited\ Coord.}$',r"${No\ CO_{2}\ Target}$"+'\n'+'$\it{Full\ Coord.}$',
#                             r"$90\%\ CO_{2}\ Cut$"+'\n'+'$\it{Limited\ Coord.}$',r"$90\%\ CO_{2}\ Cut$"+'\n'+'$\it{Full\ Coord.}$'],fontsize=8,rotation=90)
#         if ((i==2)&(j>3)):
#             ax.set_yticks([])
#             ax.set_yticklabels([])
#         if j<3:
...
# plt.show()
# #fig.savefig(figpath + 'capacity_additions.png', bbox_inches='tight', dpi=600)

#%% NEW COORDINATION BOXPLOT DIFFS (new 12/2/24)

# ### CAP ###
# capcap = new_cap[new_cap.load_zone=='Region-wide'][['scs','Wind_new','Solar_new','Fossil_new','Hydro_new','Battery_new',
#                                                     'Nuclear_new','Biomass_new','emis_policy','exist_trd','fixed_pv']].copy(deep=True)
# cps=capcap.melt(id_vars=['scs','fixed_pv'],value_vars=['Wind_new','Solar_new','Fossil_new','Hydro_new','Battery_new','Nuclear_new','Biomass_new'])
# cps['variable'] = cps['variable'].map({'Wind_new':'Wind','Solar_new':'Solar','Fossil_new':'Fossil','Hydro_new':'Hydro',
#                                         'Battery_new':'Battery','Nuclear_new':'Nuclear','Biomass_new':'Biomass'})
# cps['emis'] = cps['scs'].str.contains('cut90p')
# cps['trd'] = cps['scs'].str.contains('_tn')
# cps['rscen'] = cps['scs'].str[:-3]
# cpsm=cps[cps.trd==0].merge(cps[cps.trd==1],on=['rscen','emis','variable','fixed_pv'],
#                     suffixes=['_t1','_tn']).drop(columns=['scs_t1','scs_tn','trd_t1','trd_tn'])
# cpsm['diff'] = cpsm['value_t1'] - cpsm['value_tn']
# cpsm = cpsm[['rscen','variable','emis','fixed_pv','diff']].rename(columns={'variable':'tech'}).copy(deep=True)
# cpsm['emis'] = cpsm['emis'].map({True:'cut90p',False:'nocut'})
# cpsm['pol'] = cpsm['emis'] + cpsm['fixed_pv'].astype(str)
# ### GEN ###
...
# font.set_name('Open Sans')
# plt.rcParams["font.family"] = "Arial"
# fig = plt.figure(constrained_layout=False, figsize = (7.5,3.2))
# gs = fig.add_gridspec(1,5, height_ratios=[1], width_ratios=[1,0.04,1,0.09,1], wspace=.49)
# ax1,ax2,ax3 = fig.add_subplot(gs[0]),fig.add_subplot(gs[2]),fig.add_subplot(gs[4])
# axs, metdfs, yl, lp, pl = [ax1,ax2,ax3], [ctsm,cpsm,gmsm], [8,40,160], 4, ['(a)','(b)','(c)']
...
#     axs[i].set_xlim([-0.6,6.6])
#     axs[i].set_xlabel('')
#     axs[i].set_xticks([j for j in range(7)])
#     axs[i].set_xticklabels([r'$\bf{Wind}$',r'$\bf{Solar}$',r'$\bf{Fossil}$','Hydro','Battery','Nuclear','Biomass'])
...
# ax1.set_ylabel('Difference in '+'$\mathbf{Capacity\ Investments}$'+'\nwhen Limiting Coordination ($B)',labelpad=lp)
# ax2.set_ylabel('Difference in '+'$\mathbf{Installed\ Capacity}$'+'\nwhen Limiting Coordination (GW)',labelpad=lp)
# ax3.set_ylabel('Difference in '+'$\mathbf{Total\ Generation}$'+'\nwhen Limiting Coordination (TWh)',labelpad=lp)
# plt.show()
# #plt.savefig(figpath + 'coord_diffs_capgencost.png', bbox_inches='tight', dpi=800)
...
# ax1.set_ylabel('Impact of Full Coordination' + '\non '+'$\mathbf{Capacity\ Investments}$',labelpad=lp,fontsize=12,linespacing=1.5)
# ax2.set_ylabel('Impact of Full Coordination' + '\non '+'$\mathbf{Total\ Generation}$',labelpad=lp,fontsize=12,linespacing=1.5)

# plt.show()
# #plt.savefig(figpath + 'coord_diffs_costgen_full.png', bbox_inches='tight', dpi=800)



#%% GENERATION SHARE BAND PLOTS (SI)
...
#                 Line2D([], [], color='black', lw=1, linestyle='--')]
# ax.legend(custom_lines, [r"${No\ CO_{2}\ Target}$"+' + '+'$\it{Limited\ Coordination}$',r"${No\ CO_{2}\ Target}$"+' + '+'$\it{Full\ Coordination}$','1-Axis Solar',
#                               r"$90\%\ CO_{2}\ Cut$"+' + '+'$\it{Limited\ Coordination}$',r"$90\%\ CO_{2}\ Cut$"+' + '+'$\it{Full\ Coordination}$','Fixed Solar'],\
#                               loc='center', bbox_to_anchor=[-2.3,-0.5], fontsize=10, frameon=False, ncol=2)
# plt.show()
# #plt.savefig(figpath + 'gen_share_bands.png', bbox_inches='tight', dpi=1000)

#%% COST COMPONENT BOXPLOTS BY COUNTRY (SI)
...
# plt.show()
# #fig.savefig(figpath + 'country_costs_split_tx.png', bbox_inches='tight', dpi=600)

#%% NEW TRANSMISSION CAPACITY BOXPLOTS (SI)
...
# legend_elements = [Line2D([0], [0],label=r"${No\ CO_{2}\ Target}$" + ' + 1-Axis Solar', color=palette['000'], lw=8),
#                     Line2D([0], [0],label=r"${No\ CO_{2}\ Target}$" + ' + Fixed Solar', color=palette['001'], lw=8),
#                     Line2D([0], [0],label=r"${90\%\ CO_{2}\ Cut}$" + ' + 1-Axis Solar', color=palette['100'], lw=8),
#                     Line2D([0], [0],label=r"${90\%\ CO_{2}\ Cut}$" + ' + Fixed Solar', color=palette['101'], lw=8)]
# ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(0,1.01), frameon=False, fontsize=10,handlelength=0.9)

# ax.set_ylim(-0.03,20)
# plt.show()
# #plt.savefig(figpath + 'new_tx_cap.png', bbox_inches='tight',dpi=600)

#%% BATTERY, CURTAILMENT, EMISSIONS BY COUNTRY (SI)
...
# plt.show()
# #fig.savefig(figpath + 'grid_performance.png', bbox_inches='tight', dpi=600)

#%% VRE PREFERENCE PARALLEL AXIS MULTI-PANEL
...
#%% TRANSMISSION DISTANCE MAPS (NEED GEOPANDAS) (SI)
...
#%% HEATMAP OF BATTERY DISPATCH / CURTAILMENT (SI)
...
#%% HEATMAP OF HOURLY TRADE (TOTAL)

# tp_trade = pd.read_csv(datapath + 'total_trade_by_timepoint.csv')
# tp50 = tp_trade[tp_trade.period==2050].groupby(['timepoint','scenario']).sum().reset_index().drop(columns='period')
# df_t1 = tp50[tp50.scenario.str.contains('t1')][['timepoint','scenario','net_flow_twh']]
# df_t1.timepoint = df_t1.timepoint.astype(str)
# df_t1['Year'] = df_t1['timepoint'].str[:4].astype(int)
# df_t1['Month'] = df_t1['timepoint'].str[4:6].astype(int)
# df_t1['Hour'] = df_t1['timepoint'].str[6:].astype(int)
# df_tn = tp50[tp50.scenario.str.contains('tn')][['timepoint','scenario','net_flow_twh']]
# df_tn.timepoint = df_tn.timepoint.astype(str)
# df_tn['Year'] = df_tn['timepoint'].str[:4].astype(int)
# df_tn['Month'] = df_tn['timepoint'].str[4:6].astype(int)
# df_tn['Hour'] = df_tn['timepoint'].str[6:].astype(int)
# df_t1_avg = df_t1.groupby(['Year','Month','Hour']).mean().reset_index()
# df_tn_avg = df_tn.groupby(['Year','Month','Hour']).mean().reset_index()
# df_t1_avg_pt = df_t1_avg.pivot('Month','Hour','net_flow_twh')*1000
# df_tn_avg_pt = df_tn_avg.pivot('Month','Hour','net_flow_twh')*1000

# # hard-coded limits for plotting
# monthly_lims, moticks, day_lims, dayticks = 400, 400, 200, 200
# hr, wr, ws, hs = [1,4.5], [8,1,0.2,0.2], 0.032, 0.045

# plt.rcParams.update(plt.rcParamsDefault)
# plt.style.use('seaborn-white')
# fig = plt.figure(constrained_layout=False, figsize = (6,3))
# gs = fig.add_gridspec(2,4, height_ratios=hr, width_ratios=wr, wspace=ws,hspace=hs)
# ax_cb = fig.add_subplot(gs[:,3])
# df = df_tn_avg_pt.copy(deep=True)
# hourly_values = df.sum(axis=0).reset_index().rename(columns={'Hour':'Hour',0:'Sum'})
# monthly_values = df.sum(axis=1).reset_index().rename(columns={'Month':'Month',0:'Sum'})
# ax_hm = fig.add_subplot(gs[1,0])   #axes for heatmap
# ax_hr = fig.add_subplot(gs[0,0])   #axes for hour bars
# ax_mo = fig.add_subplot(gs[1,1])   #axes for month bars
# ax_lg = fig.add_subplot(gs[0,1])
# ax_lg.axis('off')
# h=sns.heatmap(df,ax=ax_hm,cbar_ax=ax_cb,annot=False,vmin=0,vmax=20,\
#               fmt='.0f',xticklabels=np.arange(1,25),yticklabels=np.arange(1,13))
# hour_bars = sns.barplot(x='Hour', y='Sum',data=hourly_values, ax=ax_hr, color='#88CCEE', edgecolor='k', orient='v')
# month_bars = sns.barplot(x='Sum', y='Month',data=monthly_values, ax=ax_mo, color='#88CCEE', edgecolor='k', orient='h')

# h.set_xticklabels(h.get_xmajorticklabels(), fontsize = 8, fontweight='bold')
# h.set_yticklabels(h.get_ymajorticklabels(), fontsize = 8, fontweight='bold')
# h.set_xlabel('Hour of Day', fontsize=10, fontweight='bold',labelpad=6)
# h.set_ylabel('Month', fontsize=10, fontweight='bold',labelpad=3)
# cbar = h.collections[0].colorbar
# cbar.ax.tick_params(labelsize=10)
# t = cbar.get_ticks().tolist()
# cbar.ax.set_yticks(t,labels=["{:.0f} GW".format(i) for i in t],fontweight='bold')
# ax_hr.get_xaxis().set_visible(False)
# ax_hr.set_ylim(0,day_lims)
# ax_hr.set_yticks([dayticks], fontsize=8, fontweight='bold')
# ax_hr.set_yticklabels([str(dayticks)], fontsize=8, fontweight='bold')
# ax_hr.set_ylabel('Sum', fontsize=8, fontweight='bold',labelpad=2)
# ax_mo.get_yaxis().set_visible(False)
# ax_mo.set_xlim(0,monthly_lims)
# ax_mo.set_xticks([moticks], fontsize=8, fontweight='bold')
# ax_mo.set_xticklabels([str(moticks)], fontsize=8, fontweight='bold')
# ax_mo.set_xlabel('Sum', fontsize=8, fontweight='bold',labelpad=2)
# ax_hr.set_title(r'Average Trade under $\it{Full\ Coordination}$ (2050)',fontsize=12,y=1.14)

# plt.show()

# # HEATMAP OF HOURLY TRADE (BY LINE)
# tp_trade = pd.read_csv(datapath + 'total_trade_by_timepoint_net.csv')
# tp50 = tp_trade[tp_trade.period==2050].groupby(['timepoint','tx_line','scenario']).sum().reset_index().drop(columns='period')
# df_t1 = tp50[tp50.scenario.str.contains('t1')][['timepoint','tx_line','scenario','net_flow_twh']]
# df_t1.timepoint = df_t1.timepoint.astype(str)
# df_t1['Year'] = df_t1['timepoint'].str[:4].astype(int)
# df_t1['Month'] = df_t1['timepoint'].str[4:6].astype(int)
# df_t1['Hour'] = df_t1['timepoint'].str[6:].astype(int)
# df_tn = tp50[tp50.scenario.str.contains('tn')][['timepoint','tx_line','scenario','net_flow_twh']]
# df_tn.timepoint = df_tn.timepoint.astype(str)
# df_tn['Year'] = df_tn['timepoint'].str[:4].astype(int)
# df_tn['Month'] = df_tn['timepoint'].str[4:6].astype(int)
# df_tn['Hour'] = df_tn['timepoint'].str[6:].astype(int)
# df_t1_avg = df_t1.groupby(['tx_line','Year','Month','Hour']).mean().reset_index()
...
# # hard-coded limits for plotting
# monthly_lims,moticks = [(-125,125),(0,150),(-0.5,1.5),(0,25),(0,120),(-15,15)], [[0,125],[0,150],[0,1.5],[0,25],[0,120],[0,15]]
# day_lims, dayticks = [(0,20),(-35,100),(0,0.5),(0,12),(0,50),(-1,5)], [[0,20],[0,100],[0,0.5],[0,12],[0,50],[0,5]]
# cb_mins, colorbar_maxes = [-5,-8,-0.06,-1,-5,-0.5], [5,8,0.06,1,5,0.5]
# hr, wr, ws, hs = [1,4.5], [8,1], 0.045, 0.065

# plt.rcParams.update(plt.rcParamsDefault)
# plt.style.use('seaborn-white')
# fig = plt.figure(constrained_layout=False, figsize = (12,9))
# gs = fig.add_gridspec(3,6, width_ratios=[1,0.04,0.07,0.08,1,0.04], height_ratios=[1,1,1], wspace = .13, hspace= .29)
# gs00 = gs[0,0].subgridspec(2,2, height_ratios=hr, width_ratios=wr, wspace=ws,hspace=hs)
...
# panel_labels = ['(a)','(b)','(c)','(d)','(e)','(f)']
# cbar_formats = ["{:.1f} GW","{:.0f} GW","{:.2f} GW","{:.1f} GW","{:.1f} GW","{:.2f} GW"]
# ts = [[-5,-2.5,0,2.5,5],[-8,-4,0,4,8],[-.06,-.03,0,.03,.06],[-1,-.5,0,.5,1],[-5,-2.5,0,2.5,5],[-.5,-.25,0,.25,.5]]
# for y in range(6):
#     df = df_tn_avg_list[y]
#     hourly_values = df.sum(axis=0).reset_index().rename(columns={'Hour':'Hour',0:'Sum'})
#     monthly_values = df.sum(axis=1).reset_index().rename(columns={'Month':'Month',0:'Sum'})
...
#     heatmap.set_ylabel('Month', fontsize=9, labelpad=2)
#     cbar = heatmap.collections[0].colorbar
#     cbar.ax.tick_params(labelsize=10)
...
#                         fontweight='bold',ha='center',va='center',fontsize=16)
# plt.show()
# #plt.savefig(figpath + 'heatmap_tx_lines_all_tn.png',facecolor='w',bbox_inches='tight',dpi=600)
...
#%% HEATMAP OF HOURLY TRADE SYSTEM-WIDE FOR PAIRED REF. SCENARIOS
...
# tsdf = [tsref_pt,tscut_pt]

# hr, wr, ws, hs = [1,4.5], [8,1], 0.032, 0.045
# monthly_lims,moticks = [(0,175),(0,700)], [[0,175],[0,700]]
# day_lims, dayticks = [(0,100),(0,400)], [[0,100],[0,400]]
# cb_mins, colorbar_maxes = [0,0], [7.5,30]
...
# except:
#     plt.style.use('seaborn-white')
# fig = plt.figure(constrained_layout=False, figsize = (6,6.5))
# gs = fig.add_gridspec(2,2, height_ratios=[1,1], width_ratios=[1,0.035], hspace=.24, wspace=0.08)
...
# ax_cb1, ax_cb2 = fig.add_subplot(gs[0,1]), fig.add_subplot(gs[1,1])
# gslist, cblist = [gs0,gs1], [ax_cb1,ax_cb2]
# panel_labels = ['(a)','(b)']
# cbar_formats = ["{:.1f} GW","{:.1f} GW"]
# cbl = ['Increase in Net Electricity Trade\n'+"${Reference\ (No\ CO_{2}\ Target)}$",
#         'Increase in Net Electricity Trade\n'+"${Mitigation\ (90\%\ CO_{2}\ Cut)}$"]
# ts, lp = [[0,2.5,5,0,7.5],[0,7.5,15,22.5,30]], [15.5,11]
# clmp, barc, barec = ['RdPu','YlGnBu'], ['#EC417F','#429DED'], ['#ff84b1','#0074DA']

# for y in range(2):
#     df = tsdf[y].copy(deep=True)
#     hourly_values = df.sum(axis=0).reset_index().rename(columns={'Hour':'Hour',0:'Sum'})
#     monthly_values = df.sum(axis=1).reset_index().rename(columns={'Month':'Month',0:'Sum'})
#     ax_hm = fig.add_subplot(gslist[y][1,0])   #axes for heatmap
#     ax_hr = fig.add_subplot(gslist[y][0,0])   #axes for hour bars
...
# plt.show()
# #plt.savefig(figpath + 'heatmap_total_trade_refscs.png',facecolor='w',bbox_inches='tight',dpi=800)




# ### NEW VERSION (THREE-PANEL INCLUDING DIFFERENCE BETWEEN FIRST TWO) 2/18/25
...
#%% TRADE HEATMAP THREE-PANEL, ENSEMBLE AVERAGES

# ### (FULL ENSEMBLE- THREE-PANEL INCLUDING DIFFERENCE BETWEEN FIRST TWO) 2/19/25

# tp_trade = pd.read_csv(datapath + 'total_trade_by_timepoint.csv')
# tp50 = tp_trade[tp_trade.period==2050].groupby(['timepoint','scenario']).sum().reset_index().drop(columns='period')
# tp50['t'] = tp50.scenario.str[-2:]
# tp50['e'] = tp50.scenario.str.contains('cut90p').map({True:'mit',False:'ref'})
# tp50 = tp50.drop(columns='scenario').groupby(['timepoint','t','e']).mean().reset_index()

# ts1 = tp50[(tp50.t=='t1')&(tp50.e=='ref')].copy(deep=True)
# ts2 = tp50[(tp50.t=='tn')&(tp50.e=='ref')].copy(deep=True)
# ts3 = tp50[(tp50.t=='t1')&(tp50.e=='mit')].copy(deep=True)
# ts4 = tp50[(tp50.t=='tn')&(tp50.e=='mit')].copy(deep=True)
...
# plt.show()
# #plt.savefig(figpath + 'heatmap_total_trade_allscs_3panel.png',facecolor='w',bbox_inches='tight',dpi=800)


#%% GEN HEATMAPS FOR PAIRED REF SCENARIOS

# list_pt_gen_loss_curt = []
# sc4r = ['prm15_wcost_re11_c2p1_t1','prm15_wcost_re11_c2p1_co2_cut90p_t1',
#         'prm15_wcost_re11_c2p1_tn','prm15_wcost_re11_c2p1_co2_cut90p_tn']
# for i in range(len(sc4r)):
#     if i < 2:
#         path = respath
#     else:
#         path = respath1
#     ### Generation-TWh 2020-2050
#     alldisp1 = pd.read_csv(path+''+str(sc4r[i])+'/results/dispatch_all.csv')
#     alldisp1['gw'] = alldisp1['power_mw']/(10**3)
#     alldisp1_tec_twh_nstor=alldisp1[alldisp1.operational_type!='stor'].copy(deep=True).drop(columns=['operational_type','balancing_type'])
#     alldisp1_tec_twh_nstor=alldisp1[alldisp1.period==2050].groupby(['period','horizon','timepoint','load_zone','technology'])['gw'].sum().reset_index()
#     df_gen_loss_curt = alldisp1_tec_twh_nstor.copy(deep=True)
#     ### rename tech for plotting
#     df_gen_loss_curt.loc[df_gen_loss_curt['technology'] == 'Coal_Sub','technology'] = 'Fossil'
#     df_gen_loss_curt.loc[df_gen_loss_curt['technology'] == 'Coal_Sup','technology'] = 'Fossil'
#     df_gen_loss_curt.loc[df_gen_loss_curt['technology'] == 'Coal_IGCC','technology'] = 'Fossil'
#     df_gen_loss_curt.loc[df_gen_loss_curt['technology'] == 'Gas_CCGT','technology'] = 'Fossil'
#     df_gen_loss_curt.loc[df_gen_loss_curt['technology'] == 'Gas_OCGT','technology'] = 'Fossil'
#     df_gen_loss_curt.loc[df_gen_loss_curt['technology'] == 'Gas_ICE','technology'] = 'Fossil'
#     df_gen_loss_curt.loc[df_gen_loss_curt['technology'] == 'Diesel','technology'] = 'Fossil'
#     df_gen_loss_curt.loc[df_gen_loss_curt['technology'] == 'Oil','technology'] = 'Fossil'
...
#     df_gen_loss_curt = df_gen_loss_curt[df_gen_loss_curt.technology.isin(['Fossil','Wind','Solar','Hydro','Geothermal','Biomass'])]
#     ### sum of twh by renamed tech and period
#     pt_gen_loss_curt = df_gen_loss_curt.groupby(['period','horizon','timepoint','technology'])['gw'].sum().reset_index()
#     ### assign scenario name in a column
#     pt_gen_loss_curt['scs'] = sc4r[i]
#     ### append to the pivot table to list
#     list_pt_gen_loss_curt.append(pt_gen_loss_curt)
#     print(str(i))
# dfg4r = pd.concat([list_pt_gen_loss_curt[i] for i in range(len(sc4r))]).drop(columns=['period','horizon'])

# hmt = ['Fossil','Wind','Solar','Hydro','Biomass','Geothermal']
# ggdf = []
# for i in range(len(hmt)):
#     ts1 = dfg4r[(dfg4r.scs=='prm15_wcost_re11_c2p1_t1')&(dfg4r.technology==hmt[i])].copy(deep=True)
#     ts2 = dfg4r[(dfg4r.scs=='prm15_wcost_re11_c2p1_tn')&(dfg4r.technology==hmt[i])].copy(deep=True)
#     ts3 = dfg4r[(dfg4r.scs=='prm15_wcost_re11_c2p1_co2_cut90p_t1')&(dfg4r.technology==hmt[i])].copy(deep=True)
#     ts4 = dfg4r[(dfg4r.scs=='prm15_wcost_re11_c2p1_co2_cut90p_tn')&(dfg4r.technology==hmt[i])].copy(deep=True)
#     tsref = ts1.merge(ts2,on='timepoint', suffixes=['_t1','_tn'])
#     tscut = ts3.merge(ts4,on='timepoint', suffixes=['_t1','_tn'])
#     tsref['diff'] = tsref['gw_tn'] - tsref['gw_t1']
#     tscut['diff'] = tscut['gw_tn'] - tscut['gw_t1']
#     tsref = tsref[['timepoint','diff']]
#     tscut = tscut[['timepoint','diff']]
#     tsref.timepoint = tsref.timepoint.astype(str)
#     tsref['Year'] = tsref['timepoint'].str[:4].astype(int)
...
#     tscut['Month'] = tscut['timepoint'].str[4:6].astype(int)
#     tscut['Hour'] = tscut['timepoint'].str[6:].astype(int)
#     tsref_pt = tsref.pivot(index='Month',columns='Hour',values='diff')
#     tscut_pt = tscut.pivot(index='Month',columns='Hour',values='diff')
#     ggdf.append([tsref_pt,tscut_pt])

# hr, wr, ws, hs = [1,4.5], [8,1], 0.032, 0.045
# monthly_lims,moticks = [(0,175),(0,700)], [[0,175],[0,700]]
# day_lims, dayticks = [(0,100),(0,400)], [[0,100],[0,400]]
# cb_mins, colorbar_maxes = [-20,-40],[20,40] #solar[-5,-30],[5,30] #wind[-5,-45],[5,45] #fossil[-18,-18], [18,18] #def[0,0], [7.5,30]
...
#     heatmap = sns.heatmap(df, ax=ax_hm, cbar_ax=cblist[y], annot=False, fmt='.0f',vmin=cb_mins[y], vmax=colorbar_maxes[y],\
#                           xticklabels=np.arange(1,25),yticklabels=np.arange(1,13),cmap='bwr')
#     hour_bars = sns.barplot(x='Hour', y='Sum',data=hourly_values, ax=ax_hr, color=barc[y], edgecolor=barec[y], orient='v',alpha=0.7)
#     month_bars = sns.barplot(x='Sum', y='Month',data=monthly_values, ax=ax_mo, color=barc[y], edgecolor=barec[y], orient='h',alpha=0.7)
...
# plt.show()
# #plt.savefig(figpath + 'heatmap_gen.png',facecolor='w',bbox_inches='tight',dpi=800)
