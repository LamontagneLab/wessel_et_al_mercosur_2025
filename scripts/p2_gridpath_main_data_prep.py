...
#     disp_hydro = pd.read_csv(path+''+str(scs80[i])+'/results/dispatch_gen_hydro.csv')
...
#     df_gen_loss_curt = pd.concat([alldisp1_tec_twh_ns, curt_re, curt_hydro],axis=0)
...
# solar_projects = pd.read_csv(main_path+'model_setup_misc/projdata_mercosur_solar_planned.csv').drop(columns=['index'])
# wind_projects = pd.read_csv(main_path+'model_setup_misc/projdata_mercosur_wind_planned.csv').drop(columns=['index'])
...
#     else:
#         path = respath1
#     disp_hydro = pd.read_csv(path+scs80[i]+'/results/dispatch_gen_hydro.csv')
#     disp_hydro['twh'] = disp_hydro['scheduled_curtailment_mw']*disp_hydro['timepoint_weight']/(10**6)
#     curt_hydro = disp_hydro.groupby(['period','load_zone'])['twh'].sum()
#     curt_hydro = curt_hydro.reset_index()
#     curt_hydro['technology']='curt_hydro'
#     curt_hydro['scs'] = scs80[i]
#     hyd_curt = pd.concat([hyd_curt,curt_hydro])
#     print(str(i))
# hyd_curt.to_csv(datapath + 'scheduled_hydro_curtailment.csv', index=False)
